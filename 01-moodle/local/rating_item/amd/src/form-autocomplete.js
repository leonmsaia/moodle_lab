...
define(['jquery', 'core/log', 'core/str', 'core/templates', 
    'core/notification', 'core/ajax', 'core/config'],
function($, log, str, templates, notification, ajax, cfg) {
    ...
    var changeUser = function(get_userid, options, state, newSelection){
        var promises = ajax.call([
                        { methodname: 'local_rating_item_get_user_selected_rating',
                            args: {
                                userid: get_userid
                            }
                        }
                    ...
                    promises[0].done(function(response) {
                        ...
                        context1 = $.extend({items: {label: response[0]['firstname'] +' ', 
                                value: response[0]['userid']}}, options, state);
                        
                        templates.render('local_rating_item/form_autocomplete_selection', context1)
                    .then(function(html, js) {
                        ...
                        var context = { 
                            ...
                            courseid: response[0]['courseid'],
                            cfg: cfg
                        };
                        templates.render('local_rating_item/rating_item_view_head', context)
                        .then(function(htmlhead, js) {
                            templates.replaceNodeContents($(".user-info") , htmlhead, js);
                        }).fail(function(ex) {
                            console.log(ex);
                        });
                    }).fail(function(ex) {
    ...
    var updateSelectionList = function(options, state, originalSelect) {
        ...
        originalSelect.children('option').each(function(index, ele) {
            if ($(ele).prop('selected')) {
                ...
                $("#change-right").click(function() {
                    var isLastElementSelected = $('#id_selcursos > option:selected').index() == $('#mycars > option').length - 1;

                    if (!isLastElementSelected) {
                        $('#id_selcursos > option:selected').removeAttr('selected').next('option').attr('selected', 'selected');
                    } else {
                        $('#id_selcursos > option:selected').removeAttr('selected');
                        $('#id_selcursos > option').first().attr('selected', 'selected');
                    }
                    var userIdNext = $('#id_selcursos').children("option:selected").val();
                    changeUser(userIdNext, options, state, newSelection);
                });

                $("#change-left").click(function() {
                    var isFirstElementSelected = $('#id_selcursos > option:selected').index() == 0;
                    if (!isFirstElementSelected) {
                        $('#id_selcursos > option:selected').removeAttr('selected').prev('option').attr('selected', 'selected');
                    } else {
                        $('#id_selcursos > option:selected').removeAttr('selected');
                        $('#id_selcursos > option').last().attr('selected', 'selected');
                    }
                    var userIdNext = $('#id_selcursos').children("option:selected").val();
                    changeUser(userIdNext, options, state, newSelection);
                });
        ...
        // Render the template.
        return templates.render('local_rating_item/form_autocomplete_selection', context)
        .then(function(html, js) {
            // Add it to the page.
            templates.replaceNodeContents(newSelection, html, js);
    ...
    return /** @alias module:core/form-autocomplete */ {
        ...
        enhance: function(selector, tags, ajax, placeholder, caseSensitive, showSuggestions, noSelectionString,
            ...
            var renderInput = templates.render('local_rating_item/form_autocomplete_input', context).then(function(html, js) {
                collectedjs += js;
                return html;
            });
            ...
            var renderSelection = templates.render('local_rating_item/form_autocomplete_selection', context).then(function(html, js) {
                collectedjs += js;
//                console.log(context);
                $('.selector-rating').html(html);
                return html;
