 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class actualizar_rut_facilitador extends \core\event\base {
    /**
     * Init method.
     */
    ...
    public static function get_name() {
        return "Actualizar rut facilitador";
    }
    ...
    public function get_description() {
        $message = "Actualizar rut facilitador";;
        $message .= \html_writer::empty_tag('br');
        if(!empty($this->other["error"])){
            $message .= 'error: ' . $this->other["error"];
            $message .= \html_writer::empty_tag('br');
        }
        if(!empty($this->other["data"])){
            $message .= 'data: ' . $this->other["data"];
            $message .= \html_writer::empty_tag('br');
        }
