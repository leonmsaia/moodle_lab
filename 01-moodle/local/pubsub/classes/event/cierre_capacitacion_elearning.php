 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class cierre_capacitacion_elearning extends \core\event\base {
    /**
     * Init method.
     */
    ...
    public static function get_name() {
        return "Cierre Capacitacion Elearning";
    }
    ...
    public function get_description() {
        $message = "Cierre Capacitacion Elearning";
        $message .= \html_writer::empty_tag('br');
        if(!empty($this->other["error"])){
            $message .= 'error: ' . $this->other["error"];
            $message .= \html_writer::empty_tag('br');
        }
        ...
        if(!empty($this->other["IdRegistroParticipante"])){
            $message .= 'IdRegistroParticipante: ' . $this->other["IdRegistroParticipante"];
            $message .= \html_writer::empty_tag('br');
        }
        return $message;
    }
