...
echo "Ejecutando script Aprobados Elearning X CURSOS ID $2 veces en la ruta: $1"

# Obtener total, minid y maxid solo una vez
DATA=$(php /miscursosprod45/moodle/miscursosprod/local/pubsub/cli/aprobados_total_x_cursos_id.php)
...
do   
   ...
   php /miscursosprod45/moodle/miscursosprod/local/pubsub/cli/aprobados_elearning_x_cursos_id.php \
       --lote=$i \
       --total=$2 \
       --count=$TOTAL \
       --minid=$MIN_ID \
       --maxid=$MAX_ID > $1/enviados_x_cursos_$i.txt &
done
