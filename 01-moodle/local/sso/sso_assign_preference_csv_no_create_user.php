...
$url = new moodle_url('/local/sso/sso_assign_preference_csv_no_create_user.php');
$PAGE->set_url($url);
...
if ($mform->is_cancelled()) {
    ...
    foreach ($files as $file) {
       $filename = clean_param($file->get_filename(), PARAM_FILE);
        $temppath = $tempdir . 'migrado_' . time() . '_' . str_replace(' ', '', $filename);
        file_put_contents($temppath, $file->get_content());

       // Ejecutar script CLI
         $cmd = "php {$CFG->dirroot}/local/sso/cli/process_csv_migradologin.php --file={$CFG->dirroot}/local/sso/$temppath --encoding={$formdata->encoding} --delimiter={$formdata->delimiter_name} --migrado={$formdata->migrado} --migradovalue={$formdata->migradovalue}";
