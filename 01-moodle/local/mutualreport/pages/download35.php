...
$PAGE->set_url(new \moodle_url('/local/mutualreport/pages/download35.php'));
$PAGE->set_pagetype('local_mutualreport_download'); // For formatting columns (DONT CHANGE).
...
if ($reportpersistent->get('type') === \core_reportbuilder\local\report\base::TYPE_SYSTEM_REPORT) {
    ...
    // Get the required output class and renderer name dynamically from the report instance.
    $outputclass = $systemreport->get_output_class();
    $renderername = $systemreport->get_renderer_name();

    // Instantiate the correct output class for the report.
    $outputreport = new $outputclass($reportpersistent, $systemreport, $parameters);

    /** @var \core_reportbuilder\output\renderer $renderer */
    echo $PAGE->get_renderer('local_mutualreport', $renderername)->render($outputreport);
} else {
