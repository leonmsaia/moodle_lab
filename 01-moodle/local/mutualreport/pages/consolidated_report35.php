...
use local_mutualreport\reportbuilder\local\systemreports\elsa_consolidado_v35 as report;
...
// Get migration date for the historical (3.5) report.
...
// Get migration date for the current report to use in the navlink.
...
if ($isvalid) {
    // Create report.
    /** @var \local_mutualreport\reportbuilder\local\systemreports\elsa_consolidado_v35 $report */
    $report = \core_reportbuilder\system_report_factory::create(
        report::class, $context, 'local_mutualreport', 'consolidated35', 0,
        [
    ...
    if (
            empty($userfilters['enrolment:timecreated_lte_to_cutoff_date_from']) &&
            empty($userfilters['enrolment:timecreated_lte_to_cutoff_date_to'])
        ) {
        ...
        $defaultto = $historicaltimestamp;
        $defaultfrom = $historicaltimestamp - ($daysfrom * DAYSECS);

        $defaultfilters['enrolment:timecreated_lte_to_cutoff_date_operator'] = \core_reportbuilder\local\filters\date::DATE_RANGE;
        $defaultfilters['enrolment:timecreated_lte_to_cutoff_date_from'] = $defaultfrom;
        $defaultfilters['enrolment:timecreated_lte_to_cutoff_date_to'] = $defaultto;
    }

    if (empty($userfilters['company:companyselector35_2_value'])) {
        $defaultfilters['company:companyselector35_2_operator'] = \core_reportbuilder\local\filters\select::EQUAL_TO;
        $defaultfilters['company:companyselector35_2_value'] = '';
    }
...
// Page settings.
$url = url::view_report_elsa_consolidado_v35();
$pagetitle = get_string('report_elsa_consolidado_v35', 'local_mutualreport');
$pageheading = get_string('report_elsa_consolidado_v35_heading', 'local_mutualreport');

$descdata = new \stdClass();
$descdata->date = userdate($historicaltimestamp);
$descdata->url = url::view_report_elsa_v2();
$navlinkdata = new \stdClass();
$navlinkdata->date = userdate($currenttimestamp);
$navlinkdata->url = $descdata->url;
$descdata->navlink = get_string('report_navigation_to_current', 'local_mutualreport', $navlinkdata);
...
$actionbar = new local_mutualreport\output\elsa_action_bar(
    $context, $url, $pagetitle, $pageheading, ['actionbaricon' => 'fa-compress']
);
