...
class certificados_table extends table_sql
{
    ...
    function other_cols($colname, $value)
    {
        ...
        $td = '';
        if ($colname == 'action') {
            $programa_otorgados = $DB->get_records('local_eabcprogramas_usuarios', ['certificadoid' => $value->id]);
            if ($value->status) {
        ...
        if ($colname == 'otorgados') {
            $programa_otorgados = $DB->get_records('local_eabcprogramas_usuarios', ['certificadoid' => $value->id, 'status' => utils::vigente()]);
            return count($programa_otorgados);
        }
        if ($colname == 'preview') {
            $td = '
            <a href="' . $value->urlfile . '" class="zoom" target="_blank">
                <img src="' . $value->urlfile . '" class="img-thumbnail" alt="certificado">
            </a>';
