...
require_once ($CFG->dirroot .'/local/holdingmng/classes/holdingcompanies_form.php');

//tamaño de la página
define("HOLDINGPAGESIZE", 10);
$holdingid  = optional_param('holdingid', 0, PARAM_INT);
$companies = optional_param_array('companyid', [], PARAM_INT);
$sort       = optional_param('sort', 'companyid', PARAM_TEXT);
...
$PAGE->set_url(new moodle_url($CFG->wwwroot .'/local/holdingmng/companies.php'));


//strings js
$PAGE->requires->string_for_js('holding','local_holdingmng');
$PAGE->requires->string_for_js('company','local_holdingmng');
$PAGE->requires->string_for_js('edit','local_holdingmng');
$PAGE->requires->string_for_js('deletecompanystr','local_holdingmng');
$PAGE->requires->string_for_js('nodata','local_holdingmng');
$PAGE->requires->string_for_js('confirmcompanytitle','local_holdingmng');
$PAGE->requires->string_for_js('confirmcompanymessage','local_holdingmng');
$PAGE->requires->string_for_js('remove','local_holdingmng');

//call tabulator
$PAGE->requires->js_call_amd('local_holdingmng/companies', 'init', 
...
if(is_siteadmin() or has_capability('local/holdingmng:view', context_system::instance())){
    ...
    *****************************************************************/
	...
	$mform = new holdingcompanies_form($holdingid);
	 
	//Form processing and displaying is done here
	if ($mform->is_cancelled()) {
	    //Handle form cancel operation, if cancel button is present on form
	    //show buttom back to holdings and add more copanies
		$data = [];
		...
		$link = new moodle_url('/local/holdingmng/companies.php', array('holdingid' => $holdingid, 'action' => 'create'));
		$strlink = get_string('addcompanies', 'local_holdingmng');
		$data['linkaccess'][] = ['link'=>$link, 'strlink'=>$strlink];
	    ...
	    //message
	    $errormessage = get_string('addcompanyerror', 'local_holdingmng');
		$okmessage = get_string('addcompanyok', 'local_holdingmng');

		$holdingid = $fromform->holdingid;
		$companies = (isset($fromform->companyid))?$fromform->companyid:[];
		...
		if($holdingid){
			if(is_array($companies)){
				foreach ($companies as $companyid) {
					...
					$newrow->companyid = intval($companyid);
					$newrow->timecreated = $now;
					$newrow->timemodified = $now;

				    $newrecordid = $DB->insert_record("holding_companies", $newrow, true);
				    if($newrecordid){
		...
		if($holdingid and $action == 'create'){
			$holdingobj = $DB->get_record('holding', ['id'=>$holdingid]);

			$html .= html_writer::start_tag('h2', []);
			$html .= get_string('addcompanies', 'local_holdingmng');
		...
		}elseif($holdingid and !empty($remove)){

			if($remove == 'ok'){
				$alert = ['message' => get_string('deletecompany', 'local_holdingmng')];
			}
			if($removeuser == 'fail'){
				$alert = ['message' => get_string('deletecompanyerror', 'local_holdingmng')];
			}
