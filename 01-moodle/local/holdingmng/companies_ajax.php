...
if(is_array($records) && count($records)){
	foreach ($records as $record) {
		...
		//Deleteaction
		$deluserparam = ['companyid' => $record->companyid, 'holdingid' => $holdingid];
		$deletelink = '';
		$deletelink .= '<a id="'.$record->companyid.'_'.$holdingid.'" 
		                   companyid="'. $record->companyid.'" 
		                   holdingid="'. $holdingid.'"
		                   href="javascript:void(0);"
					       class="delete-link">'.get_string('deletecompanystr', 'local_holdingmng').'</a>';

		//Company
		$companyname = '';
		$company = $DB->get_record('company', ['id'=>$record->companyid]);
		if($company){
