...
if(is_siteadmin() or has_capability('local/holdingmng:delete', context_system::instance())){
    ...
    if($holdingid && $companyid){
    	...
    	**********************************************************
    	*/ 
        $delcompanyparam = ['companyid' => $companyid, 'holdingid' => $holdingid];
    	$DB->delete_records("holding_companies", $delcompanyparam);
        
	    $record = $DB->get_record("holding_companies", $delcompanyparam);
	    if(!$record){
