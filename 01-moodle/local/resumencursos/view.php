...
$url = new moodle_url('/local/resumencursos/view.php',(array)$fromform);
...
$is_capability_str = "";
if(!is_siteadmin()){
    $facilitador = $DB->get_records('facilitador_back', ['id_user_moodle' => $USER->id]);
    $facilitador = end($facilitador);
    if(!empty($facilitador)){
        $enrolled_courses = enrol_get_all_users_courses($USER->id);
        foreach($enrolled_courses as $enrolled_course){
            if (empty(has_capability('local/resumencursos:access_course', \context_course::instance($enrolled_course->id)))){
                $is_capability = true;
...
$table = new \local_resumencursos\table\custom_table('uniqueid');
...
$mform = new \local_resumencursos\form\filters_form();
...
ob_end_clean();
echo $OUTPUT->render_from_template('local_resumencursos/tabla', array("filters" => $outputmform_body, "table_body" => $table_body, 'is_admin' => is_siteadmin(), 'active_pdf' => get_config('local_resumencursos', 'active_dpf_summary')));
