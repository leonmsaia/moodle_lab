...
/**
 * CLI script to synchronize closed sessions for a given productoid.
 *
...
if ($options['help'] || empty($options['targettype']) || empty($options['targetidentifier']) || empty($options['logid']) || empty($options['userid'])) {
    $help = <<<'EOT'
Synchronize closed sessions for a given productoid.

Options:
-t, --targettype        The type of the target (e.g., productoid). (Required)
-i, --targetidentifier  The identifier of the target. (Required)
-l, --logid             The ID of the log entry to update. (Required)
-u, --userid            The ID of the user who triggered the action. (Required)
-h, --help              Print out this help.

Example:
$ sudo -u www-data /usr/bin/php admin/tool/sessionmigrate/cli/sync_closed_sessions.php --targettype=productoid --targetidentifier=XYZ123 --logid=1 --userid=2
EOT;
    cli_writeln($help);
    exit(0);
}
...
$logentry = (object)[
    ...
    'status' => 'running',
    'timemodified' => time(),
    'message' => "Iniciando sincronización: {$targettype} = {$targetidentifier}"
];
...
try {
    ...
    $result = \tool_sessionmigrate\sessions::sync_closed_groups($targetidentifier, $db35, $logid, $userid);
