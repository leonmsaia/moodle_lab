...
/**
 * CLI script to migrate sessions for a given session GUID.
 *
...
if ($options['help'] || empty($options['targettype']) || empty($options['targetidentifier']) || empty($options['logid']) || empty($options['userid'])) {
    $help = <<<'EOT'
Migrate sessions for a given session GUID.

Options:
-t, --targettype        The type of the target (e.g., sessionguid). (Required)
-i, --targetidentifier  The identifier of the target (session GUID). (Required)
-l, --logid             The ID of the log entry to update. (Required)
-u, --userid            The ID of the user who triggered the action. (Required)
-h, --help              Print out this help.

Example:
$ sudo -u www-data /usr/bin/php admin/tool/sessionmigrate/cli/sync_sessions_by_guid.php --targettype=sessionguid --targetidentifier=53a94cd8-9673-f011-bec2-7c1e527d75de --logid=1 --userid=2
EOT;
    cli_writeln($help);
    exit(0);
}
...
$logentry = (object)[
    ...
    'status' => 'running',
    'timemodified' => time(),
    'message' => "Iniciando migración de sesión: {$targettype} = {$targetidentifier}"
];
$DB->update_record('tool_sessionmigrate_log', $logentry);

cli_heading('Migrating session for ' . $targettype . ': ' . $targetidentifier);
...
try {
    ...
    $result = \tool_sessionmigrate\sessions::migrate_session_by_guid($targetidentifier, $db35, $logid, $userid);
