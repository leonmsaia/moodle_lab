 ...
 */
class populate_planner_batch extends adhoc_task {
    ...
    public function get_name() {
        return get_string('populate_planner_batch_task', 'tool_eabcetlbridge');
    }
     ...
     * @param class-string<planner>[] $class The class of id map to use.
     * @return self
     */
    ...
    public function execute() {
        ...
        /** @var class-string<planner> $class The class of id map */
        $class = $data->class ?? false;

        $configid = $data->configid ?? false;
        ...
        if (class_exists($class)) {
            switch ($action) {
                case $class::ACTION_INSERT:
                    mtrace("Insertando registros de {$class}");
                    $class::populate_new_records(0, 1000, ['configid' => $configid]);
                    break;
                /*case $class::ACTION_DELETE:
