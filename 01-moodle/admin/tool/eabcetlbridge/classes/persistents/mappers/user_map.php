 ...
 */
class user_map extends id_map {

    const SOURCETYPE = 'user';
    const SOURCETABLE = 'user';
    const SOURCEKEY = 'username';
    const TARGETID = 'id';
    ...
    public static function get_new_records_sql() {
        ...
        $sql = "SELECT t.{$targetid}, t.{$sourcekey}
             ...
             LEFT JOIN {eabcetlbridge_id_map} $prefix
                 ...
                 WHERE {$prefix}.id IS NULL AND t.deleted = 0";
        return $sql;
    }
    ...
    /**
     * Returns a SQL query string for retrieving inconsistent records from the database that
     * are related by id, but their username does not exist, possibly because it changed.
     *
