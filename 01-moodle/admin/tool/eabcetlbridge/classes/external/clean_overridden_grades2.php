 ...
 */
class clean_overridden_grades2 extends \external_api {
    ...
    public static function execute_parameters() {
        return new external_function_parameters([
            'courseid' => new external_value(PARAM_INT, 'Optional course ID to filter grades.', VALUE_DEFAULT, 0),
            'itemid' => new external_value(PARAM_INT, 'Optional grade item ID to filter grades.', VALUE_DEFAULT, 0),
     ...
     * @param int|null $itemid Optional grade item ID.
     * @param int $startdate Optional start date.
     * @param int $enddate Optional end date.
     * @param int $limit Optional limit.
     * @return array A summary of the processing.
     */
    public static function execute($courseid = null, $itemid = null, $startdate = 0, $enddate = 0, $limit = 0) {
        global $DB;
        ...
        $fromsql = "FROM {grade_grades} gg";
        $wheresql = "WHERE gg.overridden > 0 $limitstartdatesql $limitendatesql";

        if (!empty($params['itemid'])) {
            $wheresql .= ' AND gg.itemid = :itemid';
            $sqlparams['itemid'] = $params['itemid'];
        } else if (!empty($params['courseid'])) {
            $fromsql .= " JOIN {grade_items} gi ON gi.id = gg.itemid";
            $wheresql .= ' AND gi.courseid = :courseid';
            $sqlparams['courseid'] = $params['courseid'];
        }

        $countsql = "SELECT COUNT(gg.id) $fromsql $wheresql";
        $selectsql = "SELECT gg.* $fromsql $wheresql";

        $count = $DB->count_records_sql($countsql, $sqlparams);
        ...
        $grades = [];
        $rs = $DB->get_recordset_sql($selectsql, $sqlparams, 0, $limitnum);
        foreach ($rs as $grade) {
