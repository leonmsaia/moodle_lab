...
/**
 * Eabcattendance module renderering helpers
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class eabcatt_user_sessions_cells_generator {
    ...
    /**
     * Set up params.
     * @param eabcattendance_report_data $reportdata - reportdata.
     * @param stdClass $user - user record.
     */
    public function  __construct(eabcattendance_report_data $reportdata, $user) {
        ...
        foreach ($this->reportdata->sessions as $sess) {
            ...
            !empty($this->reportdata->sessionslog[$this->user->id][$sess->id]->statusid)) {
                ...
                if ($this->user->enrolmentstart > ($sess->sessdate + $sess->duration)) {
                    $starttext = get_string('enrolmentstart', 'eabcattendance', userdate($this->user->enrolmentstart, '%d.%m.%Y'));
                    $this->construct_enrolments_info_cell($starttext);
                } else if ($this->user->enrolmentend and $this->user->enrolmentend < $sess->sessdate) {
                    $endtext = get_string('enrolmentend', 'eabcattendance', userdate($this->user->enrolmentend, '%d.%m.%Y'));
                    ...
                    $suspendext = get_string('enrolmentsuspended', 'eabcattendance', userdate($this->user->enrolmentend, '%d.%m.%Y'));
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class eabcatt_user_sessions_cells_html_generator extends eabcatt_user_sessions_cells_generator {
    ...
    protected function construct_existing_status_cell($text) {
        ...
        $this->cells[] = html_writer::span($text, 'eabcattendancestatus-'.$text);
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class eabcatt_user_sessions_cells_text_generator extends eabcatt_user_sessions_cells_generator {
 ...
 * @param int $view - EABCATT_VIEW_ALL|EABCATT_VIEW_
 * @return string.
 */
function eabcatt_construct_user_data_stat($usersummary, $view) {
    $stattable = new html_table();
    $stattable->attributes['class'] = 'attlist';
    $row = new html_table_row();
    $row->attributes['class'] = 'normal';
    $row->cells[] = get_string('sessionscompleted', 'eabcattendance') . ':';
    $row->cells[] = $usersummary->numtakensessions;
    $stattable->data[] = $row;

    $row = new html_table_row();
    $row->attributes['class'] = 'normal';
    $row->cells[] = get_string('pointssessionscompleted', 'eabcattendance') . ':';
    $row->cells[] = $usersummary->pointssessionscompleted;
    $stattable->data[] = $row;

    $row = new html_table_row();
    $row->attributes['class'] = 'normal';
    $row->cells[] = get_string('percentagesessionscompleted', 'eabcattendance') . ':';
    $row->cells[] = $usersummary->percentagesessionscompleted;
    $stattable->data[] = $row;

    if ($view == EABCATT_VIEW_ALL) {
        $row = new html_table_row();
        $row->attributes['class'] = 'highlight';
        $row->cells[] = get_string('sessionstotal', 'eabcattendance') . ':';
        $row->cells[] = $usersummary->numallsessions;
        $stattable->data[] = $row;

        $row = new html_table_row();
        $row->attributes['class'] = 'highlight';
        $row->cells[] = get_string('pointsallsessions', 'eabcattendance') . ':';
        $row->cells[] = $usersummary->pointsallsessions;
        $stattable->data[] = $row;

        $row = new html_table_row();
        $row->attributes['class'] = 'highlight';
        $row->cells[] = get_string('percentageallsessions', 'eabcattendance') . ':';
        $row->cells[] = $usersummary->allsessionspercentage;
        $stattable->data[] = $row;

        $row = new html_table_row();
        $row->attributes['class'] = 'normal';
        $row->cells[] = get_string('maxpossiblepoints', 'eabcattendance') . ':';
        $row->cells[] = $usersummary->maxpossiblepoints;
        $stattable->data[] = $row;

        $row = new html_table_row();
        $row->attributes['class'] = 'normal';
        $row->cells[] = get_string('maxpossiblepercentage', 'eabcattendance') . ':';
 ...
 *
 * @param stdclass $eabcattendance - eabcattendance record.
 * @param stdclass $user - user record
 * @return string.
 *
 */
function eabcatt_construct_full_user_stat_html_table($eabcattendance, $user) {
    $summary = new mod_eabcattendance_summary($eabcattendance->id, $user->id);
    return eabcatt_construct_user_data_stat($summary->get_all_sessions_summary_for($user->id), EABCATT_VIEW_ALL);
}
