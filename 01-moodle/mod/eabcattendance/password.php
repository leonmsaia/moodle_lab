...
$PAGE->set_title(get_string('password', 'eabcattendance'));

echo $OUTPUT->header();
echo html_writer::tag('h2', get_string('passwordgrp', 'eabcattendance'));
echo html_writer::span($session->studentpassword, 'student-password');

if (isset($session->includeqrcode) && $session->includeqrcode == 1) {
    $qrcodeurl = $CFG->wwwroot . '/mod/eabcattendance/eabcattendance.php?qrpass=' . $session->studentpassword . '&sessid=' . $session->id;
    echo html_writer::tag('h3', get_string('qrcode', 'eabcattendance'));

    $barcode = new TCPDF2DBarcode($qrcodeurl, 'QRCODE');
    $image = $barcode->getBarcodePngData(15, 15);
    echo html_writer::img('data:image/png;base64,' . base64_encode($image), get_string('qrcode', 'eabcattendance'));
}
echo $OUTPUT->footer();
