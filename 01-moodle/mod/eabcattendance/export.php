...
/**
 * Export eabcattendance sessions
 *
...
require_capability('mod/eabcattendance:export', $context);
...
$PAGE->navbar->add(get_string('export', 'eabcattendance'));

$formparams = array('course' => $course, 'cm' => $cm, 'modcontext' => $context);
$mform = new mod_eabcattendance_export_form($att->url_export(), $formparams);

if ($formdata = $mform->get_data()) {

    $pageparams = new mod_eabcattendance_page_with_filter_controls();
    $pageparams->init($cm);
    $pageparams->page = 0;
    $pageparams->group = $formdata->group;
    $pageparams->set_current_sesstype($formdata->group ? $formdata->group : mod_eabcattendance_page_with_filter_controls::SESSTYPE_ALL);
    if (isset($formdata->includeallsessions)) {
        if (isset($formdata->includenottaken)) {
            $pageparams->view = EABCATT_VIEW_ALL;
        } else {
            $pageparams->view = EABCATT_VIEW_ALLPAST;
    ...
    if ($reportdata->users) {
        $filename = clean_filename($course->shortname.'_Eabcattendances_'.userdate(time(), '%Y%m%d-%H%M'));
        ...
        if (isset($formdata->ident['id'])) {
            $data->tabhead[] = get_string('studentid', 'eabcattendance');
        }
        ...
        if (count($reportdata->sessions) > 0) {
            foreach ($reportdata->sessions as $sess) {
                $text = userdate($sess->sessdate, get_string('strftimedmyhm', 'eabcattendance'));
                $text .= ' ';
                if (!empty($sess->groupid) && empty($reportdata->groups[$sess->groupid])) {
                    $text .= get_string('deletedgroup', 'eabcattendance');
                } else {
                    $text .= $sess->groupid ? $reportdata->groups[$sess->groupid]->name : get_string('commonsession', 'eabcattendance');
                }
                $data->tabhead[] = $text;
            ...
            print_error('sessionsnotfound', 'eabcattendance', $att->url_manage());
        }
        ...
        $data->tabhead[] = get_string('takensessions', 'eabcattendance');
        $data->tabhead[] = get_string('points', 'eabcattendance');
        $data->tabhead[] = get_string('percentage', 'eabcattendance');
        $data->tabhead[] = get_string('idnumber');
        $data->tabhead[] = get_string('rut', 'eabcattendance');
        ...
        foreach ($reportdata->users as $user) {
            ...
            $cellsgenerator = new eabcatt_user_sessions_cells_text_generator($reportdata, $user);
            ...
            /*
            if(!empty(profile_user_record($user->id)->RUT)){
                $data->table[$i][] = profile_user_record($user->id)->RUT;
            }*/
            // Username como rut
            $data->table[$i][] = $user->username;
        ...
        if ($formdata->format === 'text') {
            eabcattendance_exporttocsv($data, $filename);
        } else {
            eabcattendance_exporttotableed($data, $filename, $formdata->format);
        }
        exit;
    } else {
        print_error('studentsnotfound', 'eabcattendance', $att->url_manage());
    }
}

$output = $PAGE->get_renderer('mod_eabcattendance');
$tabs = new eabcattendance_tabs($att, eabcattendance_tabs::TAB_EXPORT);
echo $output->header();
