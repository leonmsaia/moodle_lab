...
switch ($att->pageparams->action) {
    case mod_eabcattendance_sessions_page_params::ACTION_ADD:
        $url = $att->url_sessions(array('action' => mod_eabcattendance_sessions_page_params::ACTION_ADD));
        $mform = new mod_eabcattendance_add_form($url, $formparams);
        ...
        if ($formdata = $mform->get_data()) {
            $sessions = eabcattendance_construct_sessions_data_for_add($formdata, $att);
            $att->add_sessions($sessions);
            if (count($sessions) == 1) {
                $message = get_string('sessiongenerated', 'eabcattendance');
            } else {
                $message = get_string('sessionsgenerated', 'eabcattendance', count($sessions));
            }

            mod_eabcattendance_notifyqueue::notify_success($message);
            // Redirect to the sessions tab always showing all sessions.
            $SESSION->attcurrentattview[$cm->course] = EABCATT_VIEW_ALL;
            redirect($att->url_manage());
        }
        break;
    case mod_eabcattendance_sessions_page_params::ACTION_UPDATE:
        $sessionid = required_param('sessionid', PARAM_INT);

        $url = $att->url_sessions(array('action' => mod_eabcattendance_sessions_page_params::ACTION_UPDATE, 'sessionid' => $sessionid));
        $formparams['sessionid'] = $sessionid;
        $mform = new mod_eabcattendance_update_form($url, $formparams);
        ...
        if ($formdata = $mform->get_data()) {
            ...
            mod_eabcattendance_notifyqueue::notify_success(get_string('sessionupdated', 'eabcattendance'));
            redirect($att->url_manage());
        }
        $currenttab = eabcattendance_tabs::TAB_UPDATE;
        break;
    case mod_eabcattendance_sessions_page_params::ACTION_DELETE:
        $sessionid = required_param('sessionid', PARAM_INT);
        $confirm   = optional_param('confirm', null, PARAM_INT);

        if (isset($confirm) && confirm_sesskey()) {
            $att->delete_sessions(array($sessionid));
            eabcattendance_update_users_grade($att);
            redirect($att->url_manage(), get_string('sessiondeleted', 'eabcattendance'));
        }

        $sessinfo = $att->get_session_info($sessionid);

        $message = get_string('deletecheckfull', 'eabcattendance', get_string('session', 'eabcattendance'));
        $message .= str_repeat(html_writer::empty_tag('br'), 2);
        $message .= userdate($sessinfo->sessdate, get_string('strftimedmyhm', 'eabcattendance'));
        ...
        echo $OUTPUT->footer();
        exit;
    case mod_eabcattendance_sessions_page_params::ACTION_DELETE_SELECTED:
        $confirm    = optional_param('confirm', null, PARAM_INT);
        $message = get_string('deletecheckfull', 'eabcattendance', get_string('sessions', 'eabcattendance'));

        if (isset($confirm) && confirm_sesskey()) {
            $sessionsids = required_param('sessionsids', PARAM_ALPHANUMEXT);
            $sessionsids = explode('_', $sessionsids);
            if ($att->pageparams->action == mod_eabcattendance_sessions_page_params::ACTION_DELETE_SELECTED) {
        ...
        if (empty($sessid)) {
            print_error('nosessionsselected', 'eabcattendance', $att->url_manage());
        }
        ...
        echo $OUTPUT->footer();
        exit;
    case mod_eabcattendance_sessions_page_params::ACTION_CHANGE_DURATION:
        $sessid = optional_param_array('sessid', '', PARAM_SEQUENCE);
        $ids = optional_param('ids', '', PARAM_ALPHANUMEXT);

        $slist = !empty($sessid) ? implode('_', $sessid) : '';

        $url = $att->url_sessions(array('action' => mod_eabcattendance_sessions_page_params::ACTION_CHANGE_DURATION));
        $formparams['ids'] = $slist;
        $mform = new mod_eabcattendance_duration_form($url, $formparams);
        ...
        if ($formdata = $mform->get_data()) {
            ...
            redirect($att->url_manage(), get_string('sessionupdated', 'eabcattendance'));
        }
        ...
        break;
    case mod_eabcattendance_sessions_page_params::ACTION_DELETE_HIDDEN:
        $confirm  = optional_param('confirm', null, PARAM_INT);
        if ($confirm && confirm_sesskey()) {
            $sessions = $att->get_hidden_sessions();
            $att->delete_sessions(array_keys($sessions));
            redirect($att->url_manage(), get_string('hiddensessionsdeleted', 'eabcattendance'));
        }

        $a = new stdClass();
        $a->count = $att->get_hidden_sessions_count();
        $a->date = userdate($course->startdate);
        $message = get_string('confirmdeletehiddensessions', 'eabcattendance', $a);
