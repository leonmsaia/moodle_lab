...
/**
 * Defines all the backup steps that will be used by {@link backup_eabcattendance_activity_task}
 *
...
/**
 * Defines the complete eabcattendance structure for backup, with file and id annotations
 *
 * @copyright  2011 Artem Andreev <andreev.artem@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_eabcattendance_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $eabcattendance = new backup_nested_element('eabcattendance', array('id'), array(
        ...
        $session  = new backup_nested_element('session', array('id'), array(
            ...
            'description', 'descriptionformat','direction', 'directionformat', 'studentscanmark', 'studentpassword', 'autoassignstatus',
        ...
        $eabcattendance->add_child($statuses);
        $statuses->add_child($status);

        $eabcattendance->add_child($warnings);
        $warnings->add_child($warning);

        $eabcattendance->add_child($sessions);
        ...
        $eabcattendance->set_source_table('eabcattendance', array('id' => backup::VAR_ACTIVITYID));

        $status->set_source_table('eabcattendance_statuses', array('eabcattendanceid' => backup::VAR_PARENTID));

        $warning->set_source_table('eabcattendance_warning',
            array('idnumber' => backup::VAR_PARENTID));

        $session->set_source_table('eabcattendance_sessions', array('eabcattendanceid' => backup::VAR_PARENTID));

        // Data sources - user related data.
        if ($userinfo) {
            $log->set_source_table('eabcattendance_log', array('sessionid' => backup::VAR_PARENTID));
        }
        ...
        // File annotations.
        $session->annotate_files('mod_eabcattendance', 'session', 'id');

        // Return the root element (workshop), wrapped into standard activity structure.
        return $this->prepare_activity_structure($eabcattendance);
    }
}
