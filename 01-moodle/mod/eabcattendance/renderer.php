...
/**
 * Eabcattendance module renderering methods
 *
...
/**
 * Eabcattendance module renderer class
 *
 * @copyright  2011 Artem Andreev <andreev.artem@gmail.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_eabcattendance_renderer extends plugin_renderer_base {
    // External API - methods to render eabcattendance renderable components.

    /**
     * Renders tabs for eabcattendance
     *
     * @param eabcattendance_tabs $atttabs - tabs to display
     * @return string html code
     */
    protected function render_eabcattendance_tabs(eabcattendance_tabs $atttabs) {
        return print_tabs($atttabs->get_tabs(), $atttabs->currenttab, null, null, true);
    }

    /**
     * Renders filter controls for eabcattendance
     *
     * @param eabcattendance_filter_controls $fcontrols - filter controls data to display
     * @return string html code
     */
    protected function render_eabcattendance_filter_controls(eabcattendance_filter_controls $fcontrols) {
     ...
     *
     * @param eabcattendance_filter_controls $fcontrols
     * @return mixed|string
     */
    protected function render_sess_group_selector(eabcattendance_filter_controls $fcontrols) {
        switch ($fcontrols->pageparams->selectortype) {
            case mod_eabcattendance_page_with_filter_controls::SELECTOR_SESS_TYPE:
                if(is_siteadmin()) {
                    $sessgroups = $fcontrols->get_sess_groups_list();
                } else {
                    $sessgroups = \mod_eabcattendance\utils\frontutils::get_my_groups_selector();
                }

                if ($sessgroups) {
                    $select = new single_select($fcontrols->url(), 'group', $sessgroups,
                                                $fcontrols->get_current_sesstype(), null, 'selectgroup');
                    $select->label = get_string('sessions', 'eabcattendance');
                    $output = $this->output->render($select);

                    return html_writer::tag('div', $output, array('class' => 'groupselector'));
                }
                break;
            case mod_eabcattendance_page_with_filter_controls::SELECTOR_GROUP:
     ...
     * @return string
     */
    protected function render_paging_controls(eabcattendance_filter_controls $fcontrols) {
        $pagingcontrols = '';
        ...
        $totalusers = count_enrolled_users(context_module::instance($fcontrols->cm->id), 'mod/eabcattendance:canbelisted', $group);
     ...
     * @return string
     */
    protected function render_curdate_controls(eabcattendance_filter_controls $fcontrols) {
        global $CFG;

        $curdatecontrols = '';
        if ($fcontrols->curdatetxt) {
            $this->page->requires->strings_for_js(array('calclose', 'caltoday'), 'eabcattendance');
            $jsvals = array(
                    'cal_months'    => explode(',', get_string('calmonths', 'eabcattendance')),
                    'cal_week_days' => explode(',', get_string('calweekdays', 'eabcattendance')),
                    'cal_start_weekday' => $CFG->calendar_startwday,
                    'cal_cur_date'  => $fcontrols->curdate);
            $curdatecontrols = html_writer::script(js_writer::set_variable('M.eabcattendance', $jsvals));

            $this->page->requires->js('/mod/eabcattendance/calendar.js');

            $curdatecontrols .= html_writer::link($fcontrols->url(array('curdate' => $fcontrols->prevcur)),
                                                                         $this->output->larrow());
            $params = array(
                    'title' => get_string('calshow', 'eabcattendance'),
                    'id'    => 'show',
     ...
     * @return string
     */
    protected function render_view_controls(eabcattendance_filter_controls $fcontrols) {
        $views[EABCATT_VIEW_ALL] = get_string('all', 'eabcattendance');
        $views[EABCATT_VIEW_ALLPAST] = get_string('allpast', 'eabcattendance');
        $views[EABCATT_VIEW_MONTHS] = get_string('months', 'eabcattendance');
        $views[EABCATT_VIEW_WEEKS] = get_string('weeks', 'eabcattendance');
        $views[EABCATT_VIEW_DAYS] = get_string('days', 'eabcattendance');
        if ($fcontrols->reportcontrol  && $fcontrols->att->grade > 0) {
            $a = $fcontrols->att->get_lowgrade_threshold() * 100;
            $views[EABCATT_VIEW_NOTPRESENT] = get_string('below', 'eabcattendance', $a);
        }
        if ($fcontrols->reportcontrol) {
            $views[EABCATT_VIEW_SUMMARY] = get_string('summary', 'eabcattendance');
        }
        $viewcontrols = '';
    ...
    /**
     * Renders eabcattendance sessions managing table
     *
     * @param eabcattendance_manage_data $sessdata to display
     * @return string html code
     */
    protected function render_eabcattendance_manage_data(eabcattendance_manage_data $sessdata) {
     ...
     *
     * @param eabcattendance_manage_data $sessdata
     * @return string
     */
    protected function render_sess_manage_table(eabcattendance_manage_data $sessdata) {
        global $DB, $COURSE, $OUTPUT;
        $this->page->requires->js_init_call('M.mod_eabcattendance.init_manage');
        $table = new html_table();
        $table->width = '100%';
        $isstreaming = \mod_eabcattendance\metodos_comunes::is_course_streaming($COURSE->id);
        $table->head = array(
            '#',
            get_string('date'),
            get_string('time'),
            get_string('sessiontypeshort', 'eabcattendance'),
            get_string('description', 'eabcattendance'),
            get_string('actions'),
        );
        //si es curso streaming
        if($isstreaming == true){
            $table->head[] = get_string('urlsminarioweb', 'eabcattendance');
        } 
        $table->head[] = html_writer::checkbox('cb_selector', 0, false, '', array('id' => 'cb_selector'));
        $urlstreaming_flag = false;
        $table->align = array('', 'right', '', '', 'left', 'right');
        $table->size = array('1px', '1px', '1px', '', '*', '120px');
        if($isstreaming == true){
            $table->align[] = 'center';
            $table->size[] = '120px';
        }
        ...
        foreach ($sessdata->sessions as $key => $sess) {
            ...
            if ($sess->groupid) {
                if (empty($sessdata->groups[$sess->groupid])) {
                    $table->data[$sess->id][] = get_string('deletedgroup', 'eabcattendance');
                    // Remove actions and links on date/time.
                    $dta['actions'] = '';
                    $dta['date'] = userdate($sess->sessdate, get_string('strftimedmyw', 'eabcattendance'));
                    $dta['time'] = $this->construct_time($sess->sessdate, $sess->duration);
                } else {
                    $context = context_course::instance($COURSE->id);
                    $close = $DB->get_record('format_eabctiles_closegroup',array('groupid'=>$sess->groupid));                    
                    if ($close && $close->status == 1){
                        if (has_capability('mod/eabcattendance:editclose', $context)) {
                            $table->data[$sess->id][] = get_string('group') . ': ' . $sessdata->groups[$sess->groupid]->name;   
                        }else{                            
                            $table->data[$sess->id][] = get_string('finalizado','eabcattendance');
                            $dta['actions'] = '';
                        }
                    }else{
                        $table->data[$sess->id][] = get_string('group') . ': ' . $sessdata->groups[$sess->groupid]->name;
                ...
                $table->data[$sess->id][] = get_string('commonsession', 'eabcattendance');
            }
            $table->data[$sess->id][] = $sess->description;
            $table->data[$sess->id][] = $dta['actions'];
            //si es curso streaming
            if($isstreaming == true){
                $sesion_back = $DB->get_record('sesion_back', array('id_sesion_moodle' => $sess->id));
                if (!empty($sesion_back)) {
                    if (!empty($sesion_back->urlseminarioweb)) {
                        $urlstreaming_flag = true;
                        $img = html_writer::img($OUTPUT->image_url("iconmeet", "eabcattendance"), '', array('tilte' => '', 'style' => 'width: 30px'));
                        $urlstreaming = html_writer::link($sesion_back->urlseminarioweb, $img, array("target" => "_blank"));
                    }
                }
                if($urlstreaming_flag == true){
                    $table->data[$sess->id][] = $urlstreaming;
                } else {
                    $table->data[$sess->id][] = '';
                }
            }
            $table->data[$sess->id][] = html_writer::checkbox('sessid[]', $sess->id, false, '',
                                                              array('class' => 'eabcattendancesesscheckbox'));
     ...
     *
     * @param eabcattendance_password_icon $helpicon A help icon instance
     * @return string HTML fragment
     */
    protected function render_eabcattendance_password_icon(eabcattendance_password_icon $helpicon) {
        return $this->render_from_template('eabcattendance/eabcattendance_password_icon', $helpicon->export_for_template($this));
    }
    /**
     ...
     * @return array
     */
    private function construct_date_time_actions(eabcattendance_manage_data $sessdata, $sess) {
        global $DB, $USER;
        $actions = '';
        if (!empty($sess->studentpassword) &&
            (has_capability('mod/eabcattendance:manageeabcattendances', $sessdata->att->context) ||
            has_capability('mod/eabcattendance:takeeabcattendances', $sessdata->att->context) ||
            has_capability('mod/eabcattendance:changeeabcattendances', $sessdata->att->context))) {

            $icon = new eabcattendance_password_icon($sess->studentpassword, $sess->id);
            $actions .= $this->render($icon);
        }

        $date = userdate($sess->sessdate, get_string('strftimedmyw', 'eabcattendance'));
        $time = $this->construct_time($sess->sessdate, $sess->duration);
        if ($sess->lasttaken > 0) {
            if (has_capability('mod/eabcattendance:changeeabcattendances', $sessdata->att->context)) {
                $url = $sessdata->url_take($sess->id, $sess->groupid);
                $title = get_string('changeeabcattendance', 'eabcattendance');

                $date = html_writer::link($url, $date, array('title' => $title));
                $time = html_writer::link($url, $time, array('title' => $title));

                $actions .= $this->output->action_icon($url, new pix_icon('redo', $title, 'eabcattendance'));
            } else {
                $date = '<i>' . $date . '</i>';
                $time = '<i>' . $time . '</i>';
            }
        } else {
            if (has_capability('mod/eabcattendance:takeeabcattendances', $sessdata->att->context)) {
                $url = $sessdata->url_take($sess->id, $sess->groupid);
                $title = get_string('takeeabcattendance', 'eabcattendance');
                $actions .= $this->output->action_icon($url, new pix_icon('t/go', $title));
            }
        }

        if (has_capability('mod/eabcattendance:manageeabcattendances', $sessdata->att->context)) {
            $url = $sessdata->url_sessions($sess->id, mod_eabcattendance_sessions_page_params::ACTION_UPDATE);
            $title = get_string('editsession', 'eabcattendance');
            $actions .= $this->output->action_icon($url, new pix_icon('i/edit', $title));

            $url = $sessdata->url_sessions($sess->id, mod_eabcattendance_sessions_page_params::ACTION_DELETE);
            $title = get_string('deletesession', 'eabcattendance');
            $actions .= $this->output->action_icon($url, new pix_icon('t/delete', $title));

            $url = $sessdata->upload_support_attendance($sess->id, mod_eabcattendance_sessions_page_params::TAB_SUPPORT_ATTENDANCE);
            $title = get_string('uploadsupportattendance', 'eabcattendance');
            $actions .= $this->output->action_icon($url, new pix_icon('t/up', $title));

            if($sess->groupid != '0'){
                $url = $sessdata->url_sessions_download($sess->id, mod_eabcattendance_sessions_page_params::DOWNLOAD_NOMINA);
                $title = get_string('downloadnominaexcel', 'eabcattendance');
                $actions .= $this->output->action_icon($url, new pix_icon('t/download', $title));
            }
            
            if($sess->groupid != '0'){
                $url = $sessdata->url_sessions_download($sess->id, mod_eabcattendance_sessions_page_params::DOWNLOAD_NOMINA_PDF);
                $title = get_string('downloadnominapdf', 'eabcattendance');
                $actions .= $this->output->action_icon($url, new pix_icon('f/pdf', $title));
            }

            if($sess->groupid != '0'){
                $close = $DB->get_record('format_eabctiles_closegroup',array('groupid'=>$sess->groupid));
                if((!$close) || (is_siteadmin($USER))) {
                    $url = $sessdata->upload_excel_participants_attendance($sess->id, mod_eabcattendance_sessions_page_params::TAB_EXCEL_IMPORT_ATTENDANCE);
                    $title = get_string('importexcel', 'eabcattendance');
                    $actions .= $this->output->action_icon($url, new pix_icon('f/spreadsheet', $title));
                }
            }

            if($sess->groupid != '0'){
                $url = $sessdata->delete_participants_attendance($sess->groupid, $sess->id, mod_eabcattendance_sessions_page_params::TAB_DELETE_USER_ATTENDANCE);
                $title = get_string('deleteuser', 'eabcattendance');
                $actions .= $this->output->action_icon($url, new pix_icon('i/grade_incorrect', $title));
            }
         /*    $url = $sessdata->url_sessions_flags($sess->id, mod_eabcattendance_sessions_page_params::FLAGS_SESSION);
            $title = get_string('sessionsflags', 'eabcattendance');
            $actions .= $this->output->action_icon($url, new pix_icon('t/check', $title)); */
     ...
     * @return string
     */
    protected function render_sess_manage_control(eabcattendance_manage_data $sessdata) {
        $table = new html_table();
        $table->attributes['class'] = ' ';
        $table->width = '100%';
        $table->align = array('left', 'right');

        $table->data[0][] = $this->output->help_icon('hiddensessions', 'eabcattendance',
                get_string('hiddensessions', 'eabcattendance').': '.$sessdata->hiddensessionscount);
        ...
        if (has_capability('mod/eabcattendance:manageeabcattendances', $sessdata->att->context)) {
            if ($sessdata->hiddensessionscount > 0) {
                $attributes = array(
                        ...
                        'class' => 'btn btn-secondary',
                        'value' => get_string('deletehiddensessions', 'eabcattendance'));
                $table->data[1][] = html_writer::empty_tag('input', $attributes);
            }

            $options = array(mod_eabcattendance_sessions_page_params::ACTION_DELETE_SELECTED => get_string('delete'),
                mod_eabcattendance_sessions_page_params::ACTION_CHANGE_DURATION => get_string('changeduration', 'eabcattendance'));
            ...
            $controls = get_string('youcantdo', 'eabcattendance'); // You can't do anything.
        }
     ...
     * Render take data.
     *
     * @param eabcattendance_take_data $takedata
     * @return string
     */
    protected function render_eabcattendance_take_data(eabcattendance_take_data $takedata) {
        $controls = $this->render_eabcattendance_take_controls($takedata);
        $table = html_writer::start_div('no-overflow');
        if ($takedata->pageparams->viewmode == mod_eabcattendance_take_page_params::SORTED_LIST) {
            $table .= $this->render_eabcattendance_take_list($takedata);
        } else {
            $table .= $this->render_eabcattendance_take_grid($takedata);
        }
        ...
        $params = array(
                ...
                'value' => get_string('save', 'eabcattendance'));
     ...
     * Render take data.
     *
     * @param eabcattendance_delete_data $takedata
     * @return string
     */
    protected function render_eabcattendance_delete_data(eabcattendance_delete_data $takedata) {
        $controls = $this->render_eabcattendance_delete_user_controls($takedata);
        
        $table = html_writer::start_div('no-overflow');

        $table .= $this->render_eabcattendance_delete_list($takedata);
        $table .= html_writer::input_hidden_params($takedata->url(array('sesskey' => sesskey(),
                                                                        'page' => $takedata->pageparams->page,
                                                                        'perpage' => $takedata->pageparams->perpage)));
        $table .= html_writer::end_div();
     ...
     * @return string
     */
    protected function render_eabcattendance_take_controls(eabcattendance_take_data $takedata) {
        $table = new html_table();
        $table->attributes['class'] = ' ';
     ...
     * @return string
     */
    protected function render_eabcattendance_delete_user_controls(eabcattendance_delete_data $takedata) {
        $table = new html_table();
        $table->attributes['class'] = ' ';

        $table->data[0][] = $this->construct_delete_user_info($takedata);
        $table->data[0][] = $this->construct_delete_user_controls($takedata);
     ...
     * @return string
     */
    private function construct_take_session_info(eabcattendance_take_data $takedata) {
        $sess = $takedata->sessioninfo;
        $date = userdate($sess->sessdate, get_string('strftimedate'));
        $starttime = eabcattendance_strftimehm($sess->sessdate);
        $endtime = eabcattendance_strftimehm($sess->sessdate + $sess->duration);
...
/**
     ...
     * @return string
     */
    private function construct_delete_user_info(eabcattendance_delete_data $takedata) {
        $sess = $takedata->sessioninfo;
        $date = userdate($sess->sessdate, get_string('strftimedate'));
        $starttime = eabcattendance_strftimehm($sess->sessdate);
        $endtime = eabcattendance_strftimehm($sess->sessdate + $sess->duration);
     ...
     * @return string
     */
    private function construct_take_controls(eabcattendance_take_data $takedata) {

        $controls = '';
        $context = context_module::instance($takedata->cm->id);
        $group = 0;
        if ($takedata->pageparams->grouptype != mod_eabcattendance_structure::SESSION_COMMON) {
        ...
        if (!empty($takedata->cm->groupingid)) {
            ...
            $users = get_users_by_capability($context, 'mod/eabcattendance:canbelisted',
                            'u.id, u.firstname, u.lastname, u.email',
                            '', '', '', $groups,
                            '', false, true);
            $totalusers = count($users);
        } else {
            $totalusers = count_enrolled_users($context, 'mod/eabcattendance:canbelisted', $group);
        }
        ...
        if ($takedata->pageparams->grouptype == mod_eabcattendance_structure::SESSION_COMMON and
        ...
        $options = array(
            mod_eabcattendance_take_page_params::SORTED_LIST   => get_string('sortedlist', 'eabcattendance'),
            mod_eabcattendance_take_page_params::SORTED_GRID   => get_string('sortedgrid', 'eabcattendance'));
        $select = new single_select($takedata->url(), 'viewmode', $options, $takedata->pageparams->viewmode, null);
        $select->set_label(get_string('viewmode', 'eabcattendance'));
        ...
        if ($takedata->pageparams->viewmode == mod_eabcattendance_take_page_params::SORTED_LIST) {
            $options = array(
                   ...
                   get_config('eabcattendance', 'resultsperpage') => get_config('eabcattendance', 'resultsperpage'));
            $select = new single_select($takedata->url(), 'perpage', $options, $takedata->pageparams->perpage, null);
            $select->class = 'singleselect inline';
            $controls .= $this->output->render($select);
        }

        if ($takedata->pageparams->viewmode == mod_eabcattendance_take_page_params::SORTED_GRID) {
            $options = array (1 => '1 '.get_string('column', 'eabcattendance'), '2 '.get_string('columns', 'eabcattendance'),
                                   '3 '.get_string('columns', 'eabcattendance'), '4 '.get_string('columns', 'eabcattendance'),
                                   '5 '.get_string('columns', 'eabcattendance'), '6 '.get_string('columns', 'eabcattendance'),
                                   '7 '.get_string('columns', 'eabcattendance'), '8 '.get_string('columns', 'eabcattendance'),
                                   '9 '.get_string('columns', 'eabcattendance'), '10 '.get_string('columns', 'eabcattendance'));
        ...
        if (isset($takedata->sessions4copy) && count($takedata->sessions4copy) > 0) {
            ...
            foreach ($takedata->sessions4copy as $sess) {
                $start = eabcattendance_strftimehm($sess->sessdate);
                $end = $sess->duration ? ' - '.eabcattendance_strftimehm($sess->sessdate + $sess->duration) : '';
                $options[$sess->id] = $start . $end;
            }
            $select = new single_select($takedata->url(array(), array('copyfrom')), 'copyfrom', $options);
            $select->set_label(get_string('copyfrom', 'eabcattendance'));
            $select->class = 'singleselect inline';
            $controls .= $this->output->render($select);
        }
     ...
     * @return string
     */
    private function construct_delete_user_controls(eabcattendance_delete_data $takedata) {
     ...
     * @return string
     */
    protected function render_eabcattendance_take_list(eabcattendance_take_data $takedata) {
        global $PAGE, $CFG;
        $table = new html_table();
        $table->width = '0%';
        $table->head = array(
        ...
        foreach ($takedata->statuses as $st) {
            $table->head[] = html_writer::link("#", $st->description, array('id' => 'checkstatus'.$st->id,
                'title' => get_string('setallstatusesto', 'eabcattendance', $st->description)));
            $table->align[] = 'center';
            $table->size[] = '20px';
            ...
            $PAGE->requires->js_amd_inline("
                require(['jquery'], function($) {
                    $('#checkstatus".$st->id."').click(function(e) {
                        $('#eabcattendancetakeform').find('.st".$st->id."').prop('checked', true);
                        e.preventDefault();
                    });
                });");
        ...
        $table->head[] = get_string('remarks', 'eabcattendance');
        $table->align[] = 'center';
        $table->size[] = '20px';
        ...
        $row->cells[] = html_writer::div(get_string('setallstatuses', 'eabcattendance'), 'setallstatuses');
        foreach ($takedata->statuses as $st) {
            $attribs = array(
     ...
     * @return string
     */
    protected function render_eabcattendance_delete_list(eabcattendance_delete_data $takedata) {
        global $CFG;
        ...
        foreach ($takedata->users as $user) {
            ...
            $name = fullname($user);
            $url = $takedata->delete_participants_attendance($takedata->pageparams->grouptype, $takedata->pageparams->sessionid, mod_eabcattendance_sessions_page_params::DELETE_USER, $user->id, $name);
            $title = get_string('deleteuser', 'eabcattendance');
            $row->cells[] = $this->output->action_icon($url, new pix_icon('t/delete', $title));
     ...
     * @return string
     */
    protected function render_eabcattendance_take_grid(eabcattendance_take_data $takedata) {
        global $PAGE;
        $table = new html_table();
    ...
    private function construct_fullname_head($data) {
        ...
        $url = $data->url();
        if ($data->pageparams->sort == EABCATT_SORT_LASTNAME) {
            $url->param('sort', EABCATT_SORT_FIRSTNAME);
            $firstname = html_writer::link($url, get_string('firstname'));
            $lastname = get_string('lastname');
        } else if ($data->pageparams->sort == EABCATT_SORT_FIRSTNAME) {
            $firstname = get_string('firstname');
            $url->param('sort', EABCATT_SORT_LASTNAME);
            $lastname = html_writer::link($url, get_string('lastname'));
        } else {
            $firstname = html_writer::link($data->url(array('sort' => EABCATT_SORT_FIRSTNAME)), get_string('firstname'));
            $lastname = html_writer::link($data->url(array('sort' => EABCATT_SORT_LASTNAME)), get_string('lastname'));
        }
     ...
     * @return array
     */
    private function construct_take_user_controls(eabcattendance_take_data $takedata, $user) {
        $celldata = array();
        if ($user->enrolmentend and $user->enrolmentend < $takedata->sessioninfo->sessdate) {
            $celldata['text'] = get_string('enrolmentend', 'eabcattendance', userdate($user->enrolmentend, '%d.%m.%Y'));
            ...
            $celldata['text'] = get_string('enrolmentsuspended', 'eabcattendance');
            $celldata['colspan'] = count($takedata->statuses) + 1;
            $celldata['class'] = 'userwithoutenrol';
        } else {
            ...
            foreach ($takedata->statuses as $st) {
                ...
                if ($takedata->pageparams->viewmode == mod_eabcattendance_take_page_params::SORTED_GRID) {
                    $input = html_writer::tag('nobr', $input . $st->description);
                }
            ...
            if ($user->enrolmentstart > $takedata->sessioninfo->sessdate + $takedata->sessioninfo->duration) {
                $celldata['warning'] = get_string('enrolmentstart', 'eabcattendance',
     ...
     * Render header.
     *
     * @param mod_eabcattendance_header $header
     * @return string
     */
    protected function render_mod_eabcattendance_header(mod_eabcattendance_header $header) {
        if (!$header->should_render()) {
            return '';
        }

        $eabcattendance = $header->get_eabcattendance();

        $heading = format_string($header->get_title(), false, ['context' => $eabcattendance->context]);
        $o = $this->output->heading($heading);

        $o .= $this->output->box_start('generalbox boxaligncenter', 'intro');
        $o .= format_module_intro('eabcattendance', $eabcattendance, $eabcattendance->cm->id);
     ...
     * Render user data.
     *
     * @param eabcattendance_user_data $userdata
     * @return string
     */
    protected function render_eabcattendance_user_data(eabcattendance_user_data $userdata) {
        global $USER;
     ...
     * @return string
     */
    protected function render_user_report_tabs(eabcattendance_user_data $userdata) {
        $tabs = array();

        $tabs[] = new tabobject(mod_eabcattendance_view_page_params::MODE_THIS_COURSE,
                        $userdata->url()->out(true, array('mode' => mod_eabcattendance_view_page_params::MODE_THIS_COURSE)),
                        get_string('thiscourse', 'eabcattendance'));

        // Skip the 'all courses' tab for 'temporary' users.
        if ($userdata->user->type == 'standard') {
            $tabs[] = new tabobject(mod_eabcattendance_view_page_params::MODE_ALL_COURSES,
                            $userdata->url()->out(true, array('mode' => mod_eabcattendance_view_page_params::MODE_ALL_COURSES)),
                            get_string('allcourses', 'eabcattendance'));
        }
     ...
     * @return string
     */
    private function construct_user_data(eabcattendance_user_data $userdata) {
        global $USER;
        $o = '';
        if ($USER->id <> $userdata->user->id) {
        ...
        if ($userdata->pageparams->mode == mod_eabcattendance_view_page_params::MODE_THIS_COURSE) {
            $o .= $this->render_eabcattendance_filter_controls($userdata->filtercontrols);
            $o .= $this->construct_user_sessions_log($userdata);
            $o .= html_writer::empty_tag('hr');
            $o .= eabcatt_construct_user_data_stat($userdata->summary->get_all_sessions_summary_for($userdata->user->id),
                $userdata->pageparams->view);
        } else {
            $table = new html_table();
            $table->head  = array(get_string('course'),
                get_string('pluginname', 'mod_eabcattendance'),
                get_string('sessionscompleted', 'eabcattendance'),
                get_string('pointssessionscompleted', 'eabcattendance'),
                get_string('percentagesessionscompleted', 'eabcattendance'));
            ...
            foreach ($userdata->coursesatts as $ca) {
                ...
                $eabcattendanceurl = new moodle_url('/mod/eabcattendance/view.php', array('id' => $ca->cmid,
                                                                                      'studentid' => $userdata->user->id,
                                                                                      'view' => EABCATT_VIEW_ALL));
                $row->cells[] = html_writer::link($eabcattendanceurl, $ca->attname);
            ...
            $row = new html_table_row();
            if (empty($totaleabcattendance)) {
                $average = '-';
            } else {
                $average = format_float($totalpercentage / $totaleabcattendance).'%';
            }

            $col = new html_table_cell(get_string('averageeabcattendancegraded', 'mod_eabcattendance'));
            $col->attributes['class'] = 'averageeabcattendance';
            $col->colspan = 4;
            ...
            if (!empty($table2->data) && !empty($table->data)) {
                ...
                $o .= html_writer::div("<h3>".get_string('graded', 'mod_eabcattendance')."</h3>");
            }
            if (!empty($table->data)) {
            ...
            if (!empty($table2->data)) {
                ...
                $o .= html_writer::div("<h3>".get_string('ungraded', 'mod_eabcattendance')."</h3>");
     ...
     * @return string
     */
    private function construct_user_sessions_log(eabcattendance_user_data $userdata) {
        global $OUTPUT, $USER;
        ...
        if (!$shortform) {
            $table->head[] = get_string('sessiontypeshort', 'eabcattendance');
            $table->align[] = '';
            $table->size[] = '1px';
            $table->colclasses[] = '';
        }
        $table->head[] = get_string('date');
        $table->head[] = get_string('description', 'eabcattendance');
        $table->head[] = get_string('status', 'eabcattendance');
        $table->head[] = get_string('points', 'eabcattendance');
        $table->head[] = get_string('remarks', 'eabcattendance');
        ...
        if (has_capability('mod/eabcattendance:takeeabcattendances', $context)) {
            $table->head[] = get_string('action');
            $table->align[] = '';
            $table->size[] = '';
        }

        $statussetmaxpoints = eabcattendance_get_statusset_maxpoints($userdata->statuses);
        ...
        foreach ($userdata->sessionslog as $sess) {
            ...
            if (!$shortform) {
                if ($sess->groupid) {
                    ...
                    $sessiontypeshort = get_string('commonsession', 'eabcattendance');
                }

                $row->cells[] = html_writer::tag('nobr', $sessiontypeshort);
            }
            $row->cells[] = userdate($sess->sessdate, get_string('strftimedmyw', 'eabcattendance')) .
            ...
            if (!empty($sess->statusid)) {
                ...
                $cell = new html_table_cell(get_string('enrolmentstart', 'eabcattendance',
                ...
                $cell = new html_table_cell(get_string('enrolmentend', 'eabcattendance',
                                            userdate($userdata->user->enrolmentend, '%d.%m.%Y')));
                $cell->colspan = 3;
                $row->cells[] = $cell;
            } else {
                list($canmark, $reason) = eabcattendance_can_student_mark($sess, false);
                if ($canmark) {
                    // Student can mark their own eabcattendance.
                    // URL to the page that lets the student modify their eabcattendance.

                    $url = new moodle_url('/mod/eabcattendance/eabcattendance.php',
                            array('sessid' => $sess->id, 'sesskey' => sesskey()));
                    $cell = new html_table_cell(html_writer::link($url, get_string('submiteabcattendance', 'eabcattendance')));
                    $cell->colspan = 3;
                    $row->cells[] = $cell;
            ...
            if (has_capability('mod/eabcattendance:takeeabcattendances', $context)) {
                ...
                $url = new moodle_url('/mod/eabcattendance/take.php', $params);
                $icon = $OUTPUT->pix_icon('redo', get_string('changeeabcattendance', 'eabcattendance'), 'eabcattendance');
    ...
    private function construct_time($datetime, $duration) {
        $time = html_writer::tag('nobr', eabcattendance_construct_session_time($datetime, $duration));
     ...
     * Render report data.
     *
     * @param eabcattendance_report_data $reportdata
     * @return string
     */
    protected function render_eabcattendance_report_data(eabcattendance_report_data $reportdata) {
        global $PAGE, $COURSE;
        ...
        if ($reportdata->pageparams->view == EABCATT_VIEW_SUMMARY) {
            $sessionrows = array();
        } else {
        ...
        if ($bulkmessagecapability) { // Require that the user can bulk message users.
            ...
            $output .= html_writer::start_div('eabcattendancereporttable');
            ...
            $output .= html_writer::tag('div',
                    html_writer::empty_tag('input', array('type' => 'submit',
                                                                   ...
                                                                   'class' => 'btn btn-secondary')),
                    array('class' => 'buttons'));
            $url = new moodle_url('/mod/eabcattendance/messageselect.php');
    ...
    /**
     * Build and return the rows that will make up the left part of the eabcattendance report.
     * This consists of student names, as well as header cells for these columns.
     *
     * @param eabcattendance_report_data $reportdata the report data
     * @return array Array of html_table_row objects
     */
    protected function get_user_rows(eabcattendance_report_data $reportdata) {
        global $OUTPUT, $PAGE;
        $rows = array();

        $bulkmessagecapability = has_capability('moodle/course:bulkmessaging', $PAGE->context);
        $extrafields = \core_user\fields::get_identity_fields($reportdata->att->context);
        ...
        if ($extrafields) {
            if ($showextrauserdetails) {
                ...
                $text .= $OUTPUT->action_icon($url, new pix_icon('t/switch_minus',
                            get_string('hideextrauserdetails', 'eabcattendance')), null, null);
            } else {
                $params['showextrauserdetails'] = 1;
                $url = $reportdata->att->url_report($params);
                $text .= $OUTPUT->action_icon($url, new pix_icon('t/switch_plus',
                            get_string('showextrauserdetails', 'eabcattendance')), null, null);
                $extrafields = array();
            }
        }
        ...
        foreach ($reportdata->users as $user) {
            ...
            $text = '';
            if ($bulkmessagecapability) {
                $text .= html_writer::checkbox('user'.$user->id, 'on', false, '', array('class' => 'eabcattendancesesscheckbox'));
            }
        ...
        $row = new html_table_row();
        $text = ($reportdata->pageparams->view == EABCATT_VIEW_SUMMARY) ? '' : get_string('summary');
    ...
    /**
     * Build and return the rows that will make up the summary part of the eabcattendance report.
     ...
     * @return array Array of html_table_row objects
     */
    protected function get_acronym_rows(eabcattendance_report_data $reportdata, $startwithcontrast=false) {
        $rows = array();
        ...
        foreach ($reportdata->statuses as $sts) {
            if ($sts->setnumber != $setnumber) {
                $contrast = !$contrast;
                $setnumber = $sts->setnumber;
                $text = eabcattendance_get_setname($reportdata->att->id, $setnumber, false);
     ...
     * @return array Array of html_table_row objects
     */
    protected function get_summary_rows(eabcattendance_report_data $reportdata, $startwithcontrast=false) {
        $rows = array();
        ...
        $row1 = new html_table_row();
        $helpicon = $this->output->help_icon('oversessionstaken', 'eabcattendance');
        $row1->cells[] = $this->build_header_cell(get_string('oversessionstaken', 'eabcattendance') . $helpicon, $contrast, true, 3);

        $row2 = new html_table_row();
        $row2->cells[] = $this->build_header_cell(get_string('sessions', 'eabcattendance'), $contrast);
        $row2->cells[] = $this->build_header_cell(get_string('points', 'eabcattendance'), $contrast);
        $row2->cells[] = $this->build_header_cell(get_string('percentage', 'eabcattendance'), $contrast);
        $summarycells[] = $this->build_data_cell('', $contrast);
        $summarycells[] = $this->build_data_cell('', $contrast);
        $summarycells[] = $this->build_data_cell('', $contrast);
        ...
        if ($reportdata->pageparams->view == EABCATT_VIEW_SUMMARY) {
            ...
            $helpicon = $this->output->help_icon('overallsessions', 'eabcattendance');
            $row1->cells[] = $this->build_header_cell(get_string('overallsessions', 'eabcattendance') . $helpicon, $contrast, true, 3);
            ...
            $contrast = !$contrast;
            $helpicon = $this->output->help_icon('maxpossible', 'eabcattendance');
            $row1->cells[] = $this->build_header_cell(get_string('maxpossible', 'eabcattendance') . $helpicon, $contrast, true, 2);
    ...
    /**
     * Build and return the rows that will make up the eabcattendance report.
     ...
     * @return array Array of html_table_row objects
     */
    protected function get_session_rows(eabcattendance_report_data $reportdata, $startwithcontrast=false) {
        global $OUTPUT;
        ...
        $text = get_string('sessions', 'eabcattendance');
        $params = $reportdata->pageparams->get_significant_params();
        if (count($reportdata->sessions) > 1) {
            if ($showsessiondetails) {
                ...
                $url = $reportdata->att->url_report($params);
                $text .= $OUTPUT->action_icon($url, new pix_icon('t/switch_minus',
                            get_string('hidensessiondetails', 'eabcattendance')), null, null);
                $colspan = count($reportdata->sessions);
            } else {
                $params['showsessiondetails'] = 1;
                $url = $reportdata->att->url_report($params);
                $text .= $OUTPUT->action_icon($url, new pix_icon('t/switch_plus',
                            get_string('showsessiondetails', 'eabcattendance')), null, null);
                $colspan = 1;
            }
        } else {
            $colspan = 1;
        ...
        if ($reportdata->pageparams->sessiondetailspos == 'left') {
            ...
            $url = $reportdata->att->url_report($params);
            $text .= $OUTPUT->action_icon($url, new pix_icon('t/right', get_string('moveright', 'eabcattendance')), null, null);
        } else {
            $params['sessiondetailspos'] = 'left';
            $url = $reportdata->att->url_report($params);
            $text = $OUTPUT->action_icon($url, new pix_icon('t/left', get_string('moveleft', 'eabcattendance')), null, null) . $text;
        }
        ...
        if ($showsessiondetails && !empty($reportdata->sessions)) {
            foreach ($reportdata->sessions as $sess) {
                $sesstext = userdate($sess->sessdate, get_string('strftimedm', 'eabcattendance'));
                $sesstext .= html_writer::empty_tag('br');
                $sesstext .= eabcattendance_strftimehm($sess->sessdate);
                $capabilities = array(
                ...
                if (is_null($sess->lasttaken) and has_any_capability($capabilities, $reportdata->att->context)) {
                    $sesstext = html_writer::link($reportdata->url_take($sess->id, $sess->groupid), $sesstext,
                        array('class' => 'eabcattendancereporttakelink'));
                }
                $sesstext .= html_writer::empty_tag('br', array('class' => 'eabcattendancereportseparator'));
                if (!empty($sess->description) &&
                    !empty(get_config('eabcattendance', 'showsessiondescriptiononreport'))) {
                    $sesstext .= html_writer::tag('small', format_text($sess->description),
                        array('class' => 'eabcattendancereportcommon'));
                }
                if ($sess->groupid) {
                    if (empty($reportdata->groups[$sess->groupid])) {
                        $sesstext .= html_writer::tag('small', get_string('deletedgroup', 'eabcattendance'),
                            array('class' => 'eabcattendancereportgroup'));
                    } else {
                    ...
                    $sesstext .= html_writer::tag('small', get_string('commonsession', 'eabcattendance'),
                        array('class' => 'eabcattendancereportcommon'));
                }
        ...
        foreach ($reportdata->users as $user) {
            $row = new html_table_row();
            if ($showsessiondetails && !empty($reportdata->sessions)) {
                $cellsgenerator = new eabcatt_user_sessions_cells_html_generator($reportdata, $user);
     ...
     *
     * @param eabcattendance_set_selector $sel
     * @return string
     */
    protected function render_eabcattendance_set_selector(eabcattendance_set_selector $sel) {
        ...
        $opts[$newurl->out(false)] = get_string('newstatusset', 'mod_eabcattendance');
     ...
     * @return string
     */
    protected function render_eabcattendance_preferences_data($prefdata) {
        $this->page->requires->js('/mod/eabcattendance/module.js');
        ...
        $table->width = '100%';
        $table->head = array('#',
                             get_string('acronym', 'eabcattendance'),
                             get_string('description'),
                             get_string('points', 'eabcattendance'));
        $table->align = array('center', 'center', 'center', 'center', 'center', 'center');
        if ($studentscanmark) {
            $table->head[] = get_string('studentavailability', 'eabcattendance').
                $this->output->help_icon('studentavailability', 'eabcattendance');
            $table->align[] = 'center';

            $table->head[] = get_string('setunmarked', 'eabcattendance').
                $this->output->help_icon('setunmarked', 'eabcattendance');
            $table->align[] = 'center';
        }
        $table->head[] = get_string('action');
        ...
        foreach ($prefdata->statuses as $st) {
            ...
            if (isset($prefdata->errors[$st->id]) && !empty(($prefdata->errors[$st->id]))) {
                if (empty($prefdata->errors[$st->id]['acronym'])) {
                    $emptyacronym = $this->construct_notice(get_string('emptyacronym', 'mod_eabcattendance'), 'notifyproblem');
                }
                if (empty($prefdata->errors[$st->id]['description'])) {
                    $emptydescription = $this->construct_notice(get_string('emptydescription', 'mod_eabcattendance') , 'notifyproblem');
                }
            }
            $cells = array();
            $cells[] = $i;
        ...
        $table->data[$i][] = $this->construct_preferences_button(get_string('add', 'eabcattendance'),
            mod_eabcattendance_preferences_page_params::ACTION_ADD);
        ...
        $o .= $this->construct_preferences_button(get_string('update', 'eabcattendance'),
                                                  mod_eabcattendance_preferences_page_params::ACTION_SAVE);
     ...
     *
     * @param eabcattendance_default_statusset $prefdata
     * @return string
     */
    protected function render_eabcattendance_default_statusset(eabcattendance_default_statusset $prefdata) {
        return $this->render_eabcattendance_preferences_data($prefdata);
    }
     ...
     * @return string
     */
    protected function render_eabcattendance_pref($prefdata) {
    ...
    private function construct_preferences_actions_icons($st, $prefdata) {
        ...
        $params = array('sesskey' => sesskey(),
                        'statusid' => $st->id);
        if ($st->visible) {
            $params['action'] = mod_eabcattendance_preferences_page_params::ACTION_HIDE;
            $showhideicon = $OUTPUT->action_icon(
                    $prefdata->url($params),
                    new pix_icon("t/hide", get_string('hide')));
        } else {
            $params['action'] = mod_eabcattendance_preferences_page_params::ACTION_SHOW;
            $showhideicon = $OUTPUT->action_icon(
                    $prefdata->url($params),
                    new pix_icon("t/show", get_string('show')));
        }
        if (empty($st->haslogs)) {
            $params['action'] = mod_eabcattendance_preferences_page_params::ACTION_DELETE;
    ...
    private function construct_preferences_button($text, $action) {
        $attributes = array(
                ...
                'value'     => $text,
                'class'     => 'btn btn-secondary',
                'onclick'   => 'M.mod_eabcattendance.set_preferences_action('.$action.')');
        return html_writer::empty_tag('input', $attributes);
    }
    ...
    protected function user_picture($user, array $opts = null) {
        if ($user->type == 'temporary') {
            ...
            return $this->output->pix_icon('ghost', '', 'mod_eabcattendance', $attrib);
        }
