 ...
 */
function eabcattendance_create_calendar_event(&$session) {
    global $DB;
    ...
    if (empty(get_config('eabcattendance', 'enablecalendar')) || $session->calendarevent === 0) {
        // Calendar events are not used, or event not required for this session.
        return true;
    }

    $eabcattendance = $DB->get_record('eabcattendance', array('id' => $session->eabcattendanceid));

    $caleventdata = new stdClass();
    $caleventdata->name           = $eabcattendance->name;
    $caleventdata->courseid       = $eabcattendance->course;
    $caleventdata->groupid        = $session->groupid;
    $caleventdata->instance       = $session->eabcattendanceid;
    ...
    $caleventdata->eventtype      = 'eabcattendance';
    $caleventdata->timemodified   = time();
    $caleventdata->modulename     = 'eabcattendance';
    ...
    if ($calevent = calendar_event::create($caleventdata, false)) {
        ...
        $DB->set_field('eabcattendance_sessions', 'caleventid', $session->caleventid, array('id' => $session->id));
        return true;
    } else {
        return false;
    }
 ...
 */
function eabcattendance_create_calendar_events($sessionsids) {
    global $DB;
 ...
 */
function eabcattendance_update_calendar_event($session) {
    global $DB;
    ...
    if ($session->caleventid == 0) {
        return eabcattendance_create_calendar_event($session);
    }
 ...
 * @return bool result of updating
 */
function eabcattendance_delete_calendar_events($sessionsids) {
    global $DB;
    $caleventsids = eabcattendance_existing_calendar_events_ids($sessionsids);
    if ($caleventsids) {
 ...
 */
function eabcattendance_existing_calendar_events_ids($sessionsids) {
    global $DB;
    $caleventsids = array_keys($DB->get_records_list('eabcattendance_sessions', 'id', $sessionsids, '', 'caleventid'));
