 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_eabcattendance_summary {

    /** @var int eabcattendance instance identifier */
    private $eabcattendanceid;
     ...
     *
     * @param int $eabcattendanceid instance identifier
     * @param array $userids user instances identifier
     * @param int $startdate Eabcattendance sessions startdate
     * @param int $enddate Eabcattendance sessions enddate
     */
    public function __construct($eabcattendanceid, $userids=array(), $startdate = '', $enddate = '') {
        $this->eabcattendanceid = $eabcattendanceid;
    ...
    /**
     * Returns true if the corresponding eabcattendance instance is currently configure to work with grades (points)
     *
     * @return boolean
     */
    ...
    /**
     * Returns the groupmode of the corresponding eabcattendance instance
     *
     * @return int
     */
    ...
    public function get_user_taken_sessions_percentages() {
        ...
        foreach ($this->userspoints as $userid => $userpoints) {
            $percentages[$userid] = eabcattendance_calc_fraction($userpoints->points, $userpoints->maxpoints);
        }
    ...
    public function get_taken_sessions_summary_for($userid) {
        ...
        $usersummary->takensessionspercentage = eabcattendance_calc_fraction($usersummary->takensessionspoints,
    ...
    public function get_all_sessions_summary_for($userid) {
        ...
        $usersummary->allsessionspercentage = eabcattendance_calc_fraction($usersummary->takensessionspoints,
        ...
        $usersummary->maxpossiblepercentage = eabcattendance_calc_fraction(($usersummary->takensessionspoints + $deltapoints),
                                                                       $usersummary->allsessionsmaxpoints);
    ...
    private function compute_users_points($userids=array(), $startdate = '', $enddate = '') {
        ...
        list($this->course, $cm) = get_course_and_cm_from_instance($this->eabcattendanceid, 'eabcattendance');
        $this->groupmode = $cm->effectivegroupmode;

        $params = array(
            'attid'      => $this->eabcattendanceid,
            'attid2'     => $this->eabcattendanceid,
        ...
        $sql = " SELECT atl.studentid AS userid, COUNT(DISTINCT ats.id) AS numtakensessions,
                   ...
                   JOIN (SELECT setnumber, MAX(grade) AS maxgrade
                          ...
                          WHERE eabcattendanceid = :attid2
                            AND deleted = 0
                            AND visible = 1
                         GROUP BY setnumber) stm
                     ON (stm.setnumber = ats.statusset)
                   {$joingroup}
                  WHERE ats.eabcattendanceid = :attid
    ...
    private function compute_users_taken_sessions_by_acronym($userids=array(), $startdate = '', $enddate = '') {
        ...
        $sql = "SELECT atl.studentid AS userid, sts.setnumber, sts.acronym, COUNT(*) AS numtakensessions
                  FROM {eabcattendance_sessions} ats
                  JOIN {eabcattendance_log} atl ON (atl.sessionid = ats.id)
                  JOIN {eabcattendance_statuses} sts
                    ON (sts.eabcattendanceid = ats.eabcattendanceid AND
    ...
    private function compute_maxpoints_by_group_session() {
        ...
        $sql = "SELECT sess.groupid, COUNT(*) AS numsessions, SUM(stamax.maxgrade) AS maxpoints
                  FROM {eabcattendance_sessions} sess
                  JOIN (SELECT setnumber, MAX(grade) AS maxgrade
                                             FROM {eabcattendance_statuses}
                                            WHERE eabcattendanceid = :attid2
                                              AND deleted = 0
                 ...
                 WHERE sess.eabcattendanceid = :attid
