...
/**
 * Class definition for mod_eabcattendance_page_with_filter_controls
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_eabcattendance_page_with_filter_controls {
    /** No filter. */
    ...
    public function init($cm) {
        ...
        if (empty($this->defaultview)) {
            $this->defaultview = get_config('eabcattendance', 'defaultview');
        }
        $this->init_view();
        $this->init_curdate();
    ...
    public function init_start_end_date() {
        ...
        switch ($this->view) {
            ...
            case EABCATT_VIEW_MONTHS:
                ...
                break;
            case EABCATT_VIEW_ALLPAST:
                $this->startdate = 1;
                $this->enddate = time();
                break;
            case EABCATT_VIEW_ALL:
            case EABCATT_VIEW_NOTPRESENT:
                $this->startdate = 0;
                $this->enddate = 0;
                break;
            case EABCATT_VIEW_SUMMARY:
                $this->startdate = 1;
                $this->enddate = 1;
                break;
        }
    ...
    private function calc_sessgroupslist() {
        ...
        if ($allowedgroups) {
            if ($groupmode == VISIBLEGROUPS or has_capability('moodle/site:accessallgroups', $PAGE->context)) {
                $this->sessgroupslist[self::SESSTYPE_ALL] = get_string('all', 'eabcattendance');
            }
            // Show Common groups always.
            $this->sessgroupslist[self::SESSTYPE_COMMON] = get_string('commonsessions', 'eabcattendance');
