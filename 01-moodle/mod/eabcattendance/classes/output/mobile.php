...
/**
 * Contains the mobile output class for the eabcattendance
 *
...
namespace mod_eabcattendance\output;

defined('MOODLE_INTERNAL') || die();
/**
 * Mobile output class for the eabcattendance.
 *
 * @copyright 2018 Dan Marsden
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mobile {
    ...
    public static function mobile_view_activity($args) {
        ...
        // Capabilities check.
        $cm = get_coursemodule_from_id('eabcattendance', $cmid);
        ...
        $data['eabcattendance'] = $eabcattendance;
        $data['timestamp'] = time(); // Used to prevent eabcattendance session marking page to be cached.

        $data['eabcattendancefunction'] = 'mobile_user_form';
        $isteacher = false;
        if (has_capability('mod/eabcattendance:takeeabcattendances', $context)) {
            $isteacher = true;
            $data['eabcattendancefunction'] = 'mobile_teacher_form';
        }
        ...
        if (!empty($sessid) && (!empty($takenstatus) || $isteacher)) {
            $session = $DB->get_record('eabcattendance_sessions', array('id' => $sessid));
        ...
        $pageparams->mode = \mod_eabcattendance_view_page_params::MODE_THIS_COURSE;
        ...
        if ($isteacher && $canseegroupsession) {
            ...
            if (!empty($userkeys)) { // If this is a post from the teacher form.
                ...
                $data['messages'][]['string'] = 'eabcattendancesuccess';
            }
        }
        ...
        $sessions = $DB->get_records_select('eabcattendance_sessions',
            'eabcattendanceid = ? AND sessdate > ? ORDER BY sessdate', array($eabcattendance->id, $time));

        if (!empty($sessions)) {
            $userdata = new \eabcattendance_user_data($att, $USER->id, true);
            foreach ($sessions as $sess) {
                ...
                list($canmark, $reason) = eabcattendance_can_student_mark($sess);
                ...
                if ($isteacher || $canmark) {
                    $html = array('time' => strip_tags(eabcatt_construct_session_full_date_time($sess->sessdate, $sess->duration)),
                        'groupname' => '');
                    ...
                    if (!$isteacher && !empty($userdata->sessionslog['c'.$sess->id]->statusid)) {
                        ...
                        if (!$isteacher) {
                            if (!empty($sess->subnet) && !address_in_subnet(getremoteaddr(), $sess->subnet)) {
                                ...
                                $statusid = eabcattendance_session_get_highest_status($att, $sess);
                                if (empty($statusid)) {
                                    $data['showmessage'] = true;
                                    $data['messages'][]['string'] = 'eabcattendance_no_status';
                                }
                                $take = new \stdClass();
                                ...
                                if (!empty($sess->studentpassword) && $password != $sess->studentpassword) {
                                    ...
                                    if ($sess->autoassignstatus) {
                                        ...
                                        $takenstatus = eabcattendance_session_get_highest_status($att, $sess);
                                    }
        ...
        $summary = new \mod_eabcattendance_summary($att->id, array($USER->id), $att->pageparams->startdate,
        ...
        return [
            'templates' => [
                [
                    'id' => 'main',
                    'html' => $OUTPUT->render_from_template('mod_eabcattendance/mobile_view_page', $data),
                ],
            ],
            'javascript' => '',
            'otherdata' => ''
    ...
    /**
     * Returns the form to take eabcattendance for the mobile app.
     *
     * @param  array $args Arguments from tool_mobile_get_content WS
     * @return array HTML, javascript and other data
     */
    public static function mobile_user_form($args) {
        ...
        $eabcattendance    = $DB->get_record('eabcattendance', array('id' => $cm->instance), '*', MUST_EXIST);
        $attforsession = $DB->get_record('eabcattendance_sessions', array('id' => $sessid), '*', MUST_EXIST);
        $course        = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
        ...
        list($canmark, $reason) = eabcattendance_can_student_mark($attforsession, false);
        ...
        return [
            'templates' => [
                [
                    'id' => 'main',
                    'html' => $OUTPUT->render_from_template('mod_eabcattendance/mobile_user_form', $data),
                    'cache-view' => false
                ],
            ],
            'javascript' => '',
    ...
    public static function mobile_teacher_form($args) {
        ...
        $statuses = $att->get_statuses();
        $otherdata = array();
        $existinglog = $DB->get_records('eabcattendance_log',
        ...
        return [
            'templates' => [
                [
                    'id' => 'main',
                    'html' => $OUTPUT->render_from_template('mod_eabcattendance/mobile_teacher_form', $data),
                    'cache-view' => false
                ],
            ],
            'javascript' => '',
