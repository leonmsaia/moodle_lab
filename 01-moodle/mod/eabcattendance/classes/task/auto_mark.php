...
class auto_mark extends \core\task\scheduled_task {
    ...
    public function get_name() {
        ...
        return get_string('automarktask', 'mod_eabcattendance');
    }
    ...
    public function execute() {
        ...
        $sessions = $DB->get_recordset_select('eabcattendance_sessions',
        ...
        foreach ($sessions as $session) {
            if ($session->sessdate + $session->duration < $now || // If session is over.
                ...
                $setunmarked = $DB->get_field('eabcattendance_statuses', 'id',
                    array('eabcattendanceid' => $session->eabcattendanceid, 'setnumber' => $session->statusset,
                          'setunmarked' => 1, 'deleted' => 0));
                if (empty($setunmarked)) {
                    mtrace("No unmarked status configured for session id: ".$session->id);
                    continue;
                }
                if (empty($cacheatt[$session->eabcattendanceid])) {
                    $cacheatt[$session->eabcattendanceid] = $DB->get_record('eabcattendance', array('id' => $session->eabcattendanceid));
                }
                if (empty($cachecm[$session->eabcattendanceid])) {
                    $cachecm[$session->eabcattendanceid] = get_coursemodule_from_instance('eabcattendance',
                        $session->eabcattendanceid, $cacheatt[$session->eabcattendanceid]->course);
                }
                $courseid = $cacheatt[$session->eabcattendanceid]->course;
                if (empty($cachecourse[$courseid])) {
                    $cachecourse[$courseid] = $DB->get_record('course', array('id' => $courseid));
                }
                $context = \context_module::instance($cachecm[$session->eabcattendanceid]->id);
                ...
                $att = new \mod_eabcattendance_structure($cacheatt[$session->eabcattendanceid],
                    $cachecm[$session->eabcattendanceid], $cachecourse[$courseid], $context, $pageparams);

                $users = $att->get_users($session->groupid, 0);

                $existinglog = $DB->get_recordset('eabcattendance_log', array('sessionid' => $session->id));
                $updated = 0;
                    ...
                    if (empty($log->statusid)) {
                        if ($sessionover || !empty($userfirstaccess[$log->studentid])) {
                            ...
                            if (!empty($log->statusid)) {
                                $log->timetaken = $now;
                                $log->takenby = 0;
                                $log->remarks = get_string('autorecorded', 'eabcattendance');
                ...
                $newlog->remarks = get_string('autorecorded', 'eabcattendance');
                ...
                foreach ($users as $user) {
                    if ($sessionover || !empty($userfirstaccess[$user->id])) {
                        ...
                        if (!empty($newlog->statusid)) {
                            ...
                            $DB->insert_record('eabcattendance_log', $newlog);
                            $added++;
                            $donesomething = true;
                        }
                    }
