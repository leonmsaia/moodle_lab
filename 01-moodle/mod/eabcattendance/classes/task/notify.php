...
namespace mod_eabcattendance\task;
...
class notify extends \core\task\scheduled_task {
    ...
    public function get_name() {
        ...
        return get_string('notifytask', 'mod_eabcattendance');
    }
    ...
    public function execute() {
        ...
        // Get records for eabcattendance sessions that have been updated since last time this task ran.
        // Note: this returns all users for these sessions - even if the users eabcattendance wasn't changed
        // since last time we ran, before sending a notification we check to see if the users have
        // updated eabcattendance logs since last time they were notified.
        $records = eabcattendance_get_users_to_notify(array(), $orderby, true);
        ...
        foreach ($records as $record) {
            ...
            if (!empty($record->emailuser)) {
                // Only send one warning to this user from each eabcattendance in this run.
                ...
                if (empty($sentnotifications[$record->userid]) || !in_array($record->aid, $sentnotifications[$record->userid])) {
                    ...
                    // eabcattendance updated since the last time the notification was sent.
                    if (!empty($record->timesent)) {
                        $sql = "SELECT *
                              FROM {eabcattendance_log} l
                              JOIN {eabcattendance_sessions} s ON s.id = l.sessionid
                             WHERE s.eabcattendanceid = ? AND studentid = ? AND timetaken > ?";
                    ...
                    $record = eabcattendance_template_variables($record);
            ...
            // Only send one warning to this user from each eabcattendance in this run. - flag any higher percent notifications as sent.
            if (!empty($record->thirdpartyemails)) {
                ...
                foreach ($sendto as $senduser) {
                    ...
                    if (has_capability('mod/eabcattendance:warningemails', $context, $senduser)) {
                        ...
                        if (!isset($thirdpartynotifications[$senduser][$record->aid . '_' . $record->userid])) {
                            $thirdpartynotifications[$senduser][$record->aid . '_' . $record->userid]
                                = get_string('thirdpartyemailtext', 'eabcattendance', $record);
                        }
                    } else {
            ...
            $notify->timesent = $now;
            $DB->insert_record('eabcattendance_warning_done', $notify);
        }
        ...
        if (!empty($thirdpartynotifications)) {
            foreach ($thirdpartynotifications as $sendid => $notifications) {
                ...
                $emailcontent .= "\n\n".get_string('thirdpartyemailtextfooter', 'eabcattendance');
                $emailcontent = format_text($emailcontent);
                $emailsubject = get_string('thirdpartyemailsubject', 'eabcattendance');
