...
/**
 * Manage eabcattendance settings
 *
...
$pageparams = new mod_eabcattendance_preferences_page_params();
...
$maxstatusset = eabcattendance_get_max_statusset($att->id);
...
$PAGE->set_title($course->shortname. ": ".$att->name.' - '.get_string('settings', 'eabcattendance'));
$PAGE->set_heading($course->fullname);
$PAGE->force_settings_menu(true);
$PAGE->set_cacheable(true);
$PAGE->navbar->add(get_string('settings', 'eabcattendance'));
...
switch ($att->pageparams->action) {
    case mod_eabcattendance_preferences_page_params::ACTION_ADD:
        ...
        $newstatus->eabcattendanceid = $att->id;
        ...
        $status = eabcattendance_add_status($newstatus);
        if (!$status) {
            $notification = $OUTPUT->notification(get_string('cantaddstatus', 'eabcattendance'), 'error');
        }
    ...
    case mod_eabcattendance_preferences_page_params::ACTION_DELETE:
        if (eabcattendance_has_logs_for_status($att->pageparams->statusid)) {
            print_error('cantdeletestatus', 'eabcattendance', "attsettings.php?id=$id");
        }
        ...
        if (isset($confirm)) {
            ...
            redirect($att->url_preferences(), get_string('statusdeleted', 'eabcattendance'));
        }
    ...
    case mod_eabcattendance_preferences_page_params::ACTION_HIDE:
        $statuses = $att->get_statuses(false);
        $status = $statuses[$att->pageparams->statusid];
        eabcattendance_update_status($status, null, null, null, 0, $att->context, $att->cm);
        break;
    case mod_eabcattendance_preferences_page_params::ACTION_SHOW:
        $statuses = $att->get_statuses(false);
        $status = $statuses[$att->pageparams->statusid];
        eabcattendance_update_status($status, null, null, null, 1, $att->context, $att->cm);
        break;
...
$tabs = new eabcattendance_tabs($att, eabcattendance_tabs::TAB_PREFERENCES);
$prefdata = new eabcattendance_preferences_data($att, array_filter($errors));
$setselector = new eabcattendance_set_selector($att, $maxstatusset);
...
echo $OUTPUT->box(get_string('preferences_desc', 'eabcattendance'), 'generalbox eabcattendancedesc', 'notice');
