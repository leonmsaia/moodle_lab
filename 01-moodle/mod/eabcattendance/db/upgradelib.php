...
/**
 * Function to help upgrade old eabcattendance records and create calendar events.
 */
function eabcattendance_upgrade_create_calendar_events() {
    global $DB;

    $eabcattendances = $DB->get_records('eabcattendance', null, null, 'id, name, course');
    foreach ($eabcattendances as $att) {
        $sessionsdata = $DB->get_records('eabcattendance_sessions', array('eabcattendanceid' => $att->id), null,
        ...
        foreach ($sessionsdata as $session) {
            ...
            $calevent->eventtype      = 'eabcattendance';
            $calevent->timemodified   = time();
            $calevent->modulename     = 'eabcattendance';
            ...
            $DB->set_field('eabcattendance_sessions', 'caleventid', $caleventid, array('id' => $session->id));
        }
    }
}
