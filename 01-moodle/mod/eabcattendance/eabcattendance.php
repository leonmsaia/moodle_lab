...
require_once(dirname(__FILE__).'/student_eabcattendance_form.php');
...
$attforsession = $DB->get_record('eabcattendance_sessions', array('id' => $id), '*', MUST_EXIST);
$eabcattendance = $DB->get_record('eabcattendance', array('id' => $attforsession->eabcattendanceid), '*', MUST_EXIST);
$cm = get_coursemodule_from_instance('eabcattendance', $eabcattendance->id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);

// Require the user is logged in.
require_login($course, true, $cm);

list($canmark, $reason) = eabcattendance_can_student_mark($attforsession);
if (!$canmark) {
    redirect(new moodle_url('/mod/eabcattendance/view.php', array('id' => $cm->id)), get_string($reason, 'eabcattendance'));
    exit;
}
...
if (!empty($attforsession->subnet) && !address_in_subnet(getremoteaddr(), $attforsession->subnet)) {
    ...
    notice(get_string('subnetwrong', 'eabcattendance'), $url);
    exit; // Notice calls this anyway.
}

$pageparams->sessionid = $id;
$att = new mod_eabcattendance_structure($eabcattendance, $cm, $course, $PAGE->context, $pageparams);
...
if ($attforsession->autoassignstatus && empty($attforsession->studentpassword)) {
    $statusid = eabcattendance_session_get_highest_status($att, $attforsession);
    $url = new moodle_url('/mod/eabcattendance/view.php', array('id' => $cm->id));
    if (empty($statusid)) {
        print_error('eabcattendance_no_status', 'mod_eabcattendance', $url);
    }
    $take = new stdClass();
    ...
    if ($success) {
        ...
        redirect($url, get_string('studentmarked', 'eabcattendance'));
    } else {
        print_error('eabcattendance_already_submitted', 'mod_eabcattendance', $url);
    }
}
...
if (!empty($qrpass) && !empty($attforsession->autoassignstatus)) {
    ...
    if (!empty($attforsession->studentpassword) &&
        ...
        $url = new moodle_url('/mod/eabcattendance/eabcattendance.php', array('sessid' => $id, 'sesskey' => sesskey()));
        redirect($url, get_string('incorrectpassword', 'mod_eabcattendance'), null, \core\output\notification::NOTIFY_ERROR);
    }

    // Set the password and session id in the form, because they are saved in the eabcattendance log.
    $fromform->studentpassword = $qrpass;
    $fromform->sessid = $attforsession->id;

    $fromform->status = eabcattendance_session_get_highest_status($att, $attforsession);
...
// Create the form.
$mform = new mod_eabcattendance_student_eabcattendance_form(null,
        array('course' => $course, 'cm' => $cm, 'modcontext' => $PAGE->context, 'session' => $attforsession,
              'eabcattendance' => $att, 'password' => $qrpass));
