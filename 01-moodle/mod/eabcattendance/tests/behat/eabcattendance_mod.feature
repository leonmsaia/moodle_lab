@javascript @mod @uon @mod_eabcattendance
Feature: Teachers and Students can record session eabcattendance
  In order to record session eabcattendance
  As a student
  I need to be able to mark my own eabcattendance to a session
  And as a teacher
  I need to be able to mark any students eabcattendance to a session
  In order to report on session eabcattendance
  As a teacher
  I need to be able to export session eabcattendance and run reports
  In order to contact students with poor eabcattendance
  As a teacher
  I need the ability to message a group of students with low eabcattendance

  Background:
    Given the following "courses" exist:
      | fullname | shortname | summary                             | category | timecreated   | timemodified  |
      | Course 1 | C1        | Prove the eabcattendance activity works | 0        | ##yesterday## | ##yesterday## |
  ...
  Scenario: Students can mark their own eabcattendance and teacher can hide specific status from students.
    Given I log in as "teacher1"
    And I am on "Course 1" course homepage
    And I follow "Attendance"
    And I follow "Add"
    And I set the field "Allow students to record own eabcattendance" to "1"
    ...
    And I am on "Course 1" course homepage
    And I follow "Attendance"
    And I follow "Submit eabcattendance"
