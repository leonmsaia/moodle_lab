...
/**
 * Class format_eabctiles_testcase
 * @copyright  2018 David Watson {@link http://evolutioncode.uk}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class format_eabctiles_test extends \advanced_testcase {
     ...
     * @covers \format_eabctiles\local\format_option::set
     ...
     * @covers \format_eabctiles::update_course_format_options
    ...
    public function test_update_course_format_options(): void {
        ...
        $course = $this->getDataGenerator()->create_course(
            $this->tilescourseformatoptions,
            ['createsections' => true]);
        set_config('followthemecolour', 0, 'format_eabctiles');
        set_config('allowsubtilesview', 0, 'format_eabctiles');
        ...
        $dbdata = $DB->get_records(
            ...
            ['format' => 'eabctiles', 'courseid' => $course->id, 'sectionid' => 0]
        );
        $newvalues = [];
     ...
     * @covers \format_eabctiles_inplace_editable
     */
    public function test_update_inplace_editable(): void {
        ...
        try {
            \core_external::update_inplace_editable('format_eabctiles', 'sectionname', $section->id, 'New section name');
        ...
        $res = \core_external::update_inplace_editable('format_eabctiles', 'sectionname', $section->id, 'New section name');
    ...
    public function test_inplace_editable(): void {
        ...
        // Call callback format_eabctiles_inplace_editable() directly.
        $tmpl = component_callback('format_eabctiles', 'inplace_editable', ['sectionname', $section->id, 'Rename me again']);
     ...
     * @covers \format_eabctiles\local\video_cm::check_modify_embedded_url
     */
    public function test_video_urls(): void {
        $this->resetAfterTest();
        $this->assertEquals(
            'https://www.youtube.com/embed/abcdefghijk',
            \format_eabctiles\local\video_cm::check_modify_embedded_url('https://www.youtube.com/watch?v=abcdefghijk')
        );

        $this->assertEquals(
            'https://www.youtube.com/embed/abcdefghijk',
            \format_eabctiles\local\video_cm::check_modify_embedded_url('https://youtu.be/abcdefghijk')
        );

        $this->assertEquals(
            'https://www.youtube.com/embed/abcdefghijk',
            \format_eabctiles\local\video_cm::check_modify_embedded_url('https://www.youtube.com/shorts/abcdefghijk')
        );

        $this->assertEquals(
            'https://player.vimeo.com/video/347119375',
            \format_eabctiles\local\video_cm::check_modify_embedded_url('https://vimeo.com/347119375')
        );

        $this->assertTrue(
            \format_eabctiles\local\video_cm::is_video_url('https://www.youtube.com/embed/abcdefghijk?t=123')
        );

        $this->assertTrue(
            \format_eabctiles\local\video_cm::is_video_url(
        ...
        $this->assertEquals(
            null,
            \format_eabctiles\local\video_cm::check_modify_embedded_url(
                'https://www.youtube.com/shorts/abcdefghijk?t=4&feature=share'
            )
        );
     ...
     * @covers \format_eabctiles\local\modal_helper::get_resource_modal_cmids
     * @covers \format_eabctiles\local\modal_helper::cm_modal_type
     */
    public function test_modal_resource_cmids(): void {
        ...
        $pdffilepath = 'course/format/eabctiles/tests/fixtures/test.pdf';
        ...
        $this->assertFalse(
            \format_eabctiles\local\modal_helper::cm_has_modal($course->id, $instance->cmid)
        );
        ...
        $this->assertEquals(
            'pdf', \format_eabctiles\local\modal_helper::cm_modal_type($course->id, $instance->cmid)
        );
        ...
        \format_eabctiles\local\modal_helper::clear_cache_modal_cmids($course->id, 'resource');
        $this->assertFalse(
            \format_eabctiles\local\modal_helper::cm_has_modal($course->id, $instance->cmid)
        );
        ...
        $instance = $generator->create_instance([
            'course' => $course->id,
            'uploaded' => true,
            'defaultfilename' => 'course/format/eabctiles/tests/fixtures/test.html',
        ]);
        $this->assertEquals(
            'html', \format_eabctiles\local\modal_helper::cm_modal_type($course->id, $instance->cmid)
        );
    }

    /**
     * Test modal helper CM ID getter for URLs.
     * @covers \format_eabctiles\local\modal_helper::get_modal_allowed_cm_ids
     * @covers \format_eabctiles\local\modal_helper::cm_modal_type
     */
    public function test_modal_url_cmids(): void {
        ...
        $this->assertEquals(
            'url', \format_eabctiles\local\modal_helper::cm_modal_type($course->id, $instance->cmid)
        );

        $DB->set_field('url', 'display', RESOURCELIB_DISPLAY_POPUP, ['id' => $instance->id]);
        rebuild_course_cache($course->id, true);
        \format_eabctiles\local\modal_helper::clear_cache_modal_cmids($course->id, 'url');
        $this->assertFalse(
            \format_eabctiles\local\modal_helper::cm_has_modal($course->id, $instance->cmid)
        );
        ...
        set_config('modalresources', '', 'format_eabctiles');
        $this->assertFalse(
            \format_eabctiles\local\modal_helper::cm_has_modal($course->id, $instance->cmid)
        );
    ...
    public function test_modal_page_cmids(): void {
        ...
        $this->assertEquals(
            'page', \format_eabctiles\local\modal_helper::cm_modal_type($course->id, $instance->cmid)
        );

        set_config('modalmodules', '', 'format_eabctiles');
        $this->assertFalse(
            \format_eabctiles\local\modal_helper::cm_has_modal($course->id, $instance->cmid)
        );
    }
    ...
    public function test_modal_cmids_cache_clear(): void {
        ...
        $this->assertFalse(
            \format_eabctiles\local\modal_helper::clear_cache_modal_cmids($course->id, 'label')
        );
        // We do store data for others.
        foreach (['resource', 'url', 'page'] as $modname) {
            $this->assertTrue(
                \format_eabctiles\local\modal_helper::clear_cache_modal_cmids($course->id, $modname)
            );
        }
    }
}
