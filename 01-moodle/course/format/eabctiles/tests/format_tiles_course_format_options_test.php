...
/**
 * Class format_eabctiles_course_format_options_testcase
 * @copyright  2024 David Watson {@link http://evolutioncode.uk}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class format_eabctiles_course_format_options_test extends \advanced_testcase {
     ...
     * @covers \format_eabctiles\local\format_option::migrate_legacy_format_options
     * @return void
    ...
    public function test_course_format_option_migration(): void {
        global $DB, $CFG;
        require_once($CFG->dirroot . '/course/format/eabctiles/tests/helperlib.php');

        // Create test course old format and migrate it.
        $this->resetAfterTest(true);

        $generator = $this->getDataGenerator();
        $course = $generator->create_course(['format' => 'eabctiles', 'numsections' => 15]);
        ...
        for ($sectionnumber = 1; $sectionnumber <= 9; $sectionnumber++) {
            ...
            $file = $fs->get_file(
                $context->id, 'format_eabctiles', 'tilephoto', $sectionid, '/tilephoto/', $expectedfilename
            );
     ...
     * @covers \backup_format_eabctiles_plugin
     * @covers \restore_format_eabctiles_plugin
        ...
        for ($i = 1; $i <= 5; $i++) {
            ...
            $actualfiles = $DB->get_records_sql(
                "SELECT cs.section, f.filename
                    ...
                    AND f.component = 'format_eabctiles' AND f.filearea = 'tilephoto'
