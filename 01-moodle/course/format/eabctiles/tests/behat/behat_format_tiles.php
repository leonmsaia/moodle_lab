 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_format_eabctiles extends behat_base {

    /**
     * Set course format option for subtiles on for course.
     *
     * @Given /^format_eabctiles subtiles are on for course "(?P<coursefullname_string>(?:[^"]|\\")*)"$/
     * @param string $coursefullname
     * @throws dml_exception
     */
    public function format_eabctiles_sub_tiles_are_on_for_course($coursefullname) {
     ...
     * @return void
     */
    public function format_eabctiles_tile_has_colour(int $tilenumber, string $expectedvalue) {
        ...
        $jsscript = "(() => {return document.querySelector('style#format-eabctiles-dynamic-css') !== null;})();";
     ...
     * @return void
     */
    public function format_eabctiles_tile_has_icon(int $tilenumber, string $icon) {
     ...
     * @return void
     */
    public function format_eabctiles_js_config_exists_on_page() {
        $script = "(() => {return document.getElementById('format-eabctiles-js-config') !== null;})()";
        $result = $this->getSession()->evaluateScript($script);
        if (!$result) {
            throw new \Behat\Mink\Exception\ExpectationException(
     ...
     *
     * @Given /^format_eabctiles subtiles are off for course "(?P<coursefullname_string>(?:[^"]|\\")*)"$/
     * @param string $coursefullname
     * @throws dml_exception
     */
    public function format_eabctiles_sub_tiles_are_off_for_course($coursefullname) {
     ...
     *
     * @Given /^format_eabctiles progress indicator is showing as "(?P<progresstype_string>(?:[^"]|\\")*)" for course "(?P<coursefullname_string>(?:[^"]|\\")*)"$/
     ...
     *
     * @Then /^format_eabctiles progress for "(?P<modtype_string>(?:[^"]|\\")*)" called "(?P<activitytitle_string>(?:[^"]|\\")*)" in "(?P<coursefullname_string>(?:[^"]|\\")*)" is "(?P<value>\d+)" in the database$/
     ...
     * @return bool
     */
    public function activity_in_format_eabctiles_is_dimmed($activityname) {
     ...
     * @return bool
     */
    public function activity_in_format_eabctiles_is_not_dimmed($activityname) {
        return !$this->activity_in_format_eabctiles_is_dimmed($activityname);
    }
     ...
     * @throws Exception
     */
    public function click_format_eabctiles_activity($activityname) {
     ...
     *
     * @Given /^format_eabctiles progress indicator for tile "(?P<tilenumber>\d+)" is "(?P<numcomplete>\d+)" out of "(?P<outof>\d+)"$/
     * @param string $tilenumber
    ...
    private function i_show_hide($sectionnumber, $showhide, $xpath) {
        ...
        $strhide = get_string($showhide, 'format_eabctiles');
    ...
    public function tile_should_show_photo($coursename, $sectionnumber, $photoname) {
        ...
        $tilephoto = new \format_eabctiles\local\tile_photo($context, $sectionid);
        ...
        $imageurl = $CFG->wwwroot . "/pluginfile.php/" . $context->id
            . '/format_eabctiles/tilephoto/' . $sectionid . '/tilephoto/' . $photoname;

        $tilestyle = get_config('format_eabctiles', 'eabctilestyle');
    ...
    public function tile_should_show_no_photo($coursename, $sectionnumber) {
        ...
        $photo = \format_eabctiles\local\format_option::get(
            $courseid, format_eabctiles\local\format_option::OPTION_SECTION_PHOTO, $sectionid
        );
        if ($photo) {
            throw new \Behat\Mink\Exception\ExpectationException(
    ...
    public function should_see_section_confirm_delete($sectionname) {
        // @codingStandardsIgnoreEnd.
        $moodlerelease = \format_eabctiles\local\util::get_moodle_release();
