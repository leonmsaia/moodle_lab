@format @format_eabctiles @format_eabctiles_course_mod_visibility_nosubtiles_hidden_sec @javascript
Feature:  In a section for Teacher, hidden activities are dimmed where subtiles are not being used.
  ...
  Background:
    ...
    And format_eabctiles subtiles are off for course "Course Mod Vis NoSubtiles Course2"
