@format @format_eabctiles @format_eabctiles_mod_modal @format_eabctiles_html_modal_teacher @javascript @_file_upload
Feature: HTML file can be set to open in modal windows with subtiles off
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | resource | Test HTML file | File description      | C1     | pdf1     | 1       | 1       | 1          | course/format/eabctiles/tests/fixtures/test.html | 1        |
...
#    TODO test that we can see "Test HTML file content" too (is in embedded HTML document virtual element)?
    ...
    And format_eabctiles progress for "resource" called "Test HTML file" in "Course 1" is "0" in the database
    And I click on "Mark as done" "button" in the "Test HTML file" "dialogue"
    And I wait until the page is ready
    And format_eabctiles progress for "resource" called "Test HTML file" in "Course 1" is "1" in the database
