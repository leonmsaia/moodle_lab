...
/**
 * Specialised backup for format_eabctiles
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_format_eabctiles_plugin extends backup_format_plugin {
    ...
    protected function define_section_plugin_structure() {
        ...
        $plugin = $this->get_plugin_element(null, $this->get_format_condition(), 'eabctiles');

        // Define each element separated.
        // Adds tag <plugin_format_eabctiles_section> within section.xml files of backup file with filename / icon name.
        // The "id" field below is the mdl_format_eabctiles_tile_options.id in the old course.
        // It will appear in section.xml as <plugin_format_eabctiles_section sectionid="{sectionid}">
        // This is then used in the restore section as $data['sectionid'] in process_tiles_section();
        // The optiontype and optionvalue fields will be included in each <plugin_format_eabctiles_section> tag.
        $sectionoptions = new backup_nested_element(
            $this->get_recommended_name(), // Expect 'plugin_format_eabctiles_section'.
            ['sectionid'],
            ['optiontype', 'optionvalue', 'eabctilesversion']
        );

        // Define sources.
        $const1 = \format_eabctiles\local\format_option::OPTION_SECTION_PHOTO;
        $const2 = \format_eabctiles\local\format_option::OPTION_SECTION_ICON;

        // Include tiles version so we can use it on restore.
        $tilesversion  = filter_var(get_config('format_eabctiles', 'version') ?? 0, FILTER_SANITIZE_NUMBER_INT);

        $sectionoptions->set_source_sql(
            "SELECT elementid as sectionid, optiontype, optionvalue, $tilesversion AS tilesversion
                FROM {format_eabctiles_tile_options}
    ...
    private function pre_backup_steps() {
        ...
        // There is an observer \format_eabctiles\observer\course_backup_created() which reverses this once backup is complete.
        \format_eabctiles\local\format_option::set_legacy_format_options($courseid);
    }
    ...
    private function pre_step_fail_if_course_includes_excess_sections(int $courseid) {
        ...
        $maxsectionsconfig = \format_eabctiles\local\course_section_manager::get_max_sections();
        ...
        if ($isadmin) {
            $admintoolsurl = \format_eabctiles\local\course_section_manager::get_list_problem_courses_url();
        ...
        if ($countsections && $countsections > $maxallowed * 5) {
            ...
            \core\notification::error(get_string('restoretoomanysections', 'format_eabctiles', $a) . $admintoolsbutton);
            throw new moodle_exception('backupfailed', 'format_eabctiles', $admintoolsurl);
        }
