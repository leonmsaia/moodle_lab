...
/**
 * Specialised restore for format_eabctiles (based on the equivalent for format_topics
 *
...
/**
 * Specialised restore for format_eabctiles
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_format_eabctiles_plugin extends restore_format_plugin {
    ...
    public function define_section_plugin_structure() {
        ...
        return [new restore_path_element('eabctiles_section', $this->get_pathfor('/'))];
    }
        ...
        if ($this->is_tiles_course()) {
            ...
            if (!$needsunmappedfilesadded && version_compare($backuprelease, '4.3', '<=')) {
                ...
                $hasmappedfiles = $DB->record_exists_sql(
                    "SELECT fo.id
                        FROM {format_eabctiles_tile_options} fo
                        JOIN {files} f ON f.itemid = fo.elementid AND f.contextid = :coursecontextid
                        AND f.component = 'format_eabctiles' AND f.filearea = 'tilephoto' AND f.filepath = '/tilephoto/'
            ...
            $currentfilterbarsetting = $DB->get_record(
                'course_format_options',
                ['name' => 'displayfilterbar', 'format' => 'eabctiles', 'courseid' => $newcourseid]
            );
            ...
            if ($isoutcomesfilter) {
                ...
                $DB->delete_records(
                    'course_format_options',
                    ['name' => 'tileoutcomeid', 'format' => 'eabctiles', 'courseid' => $newcourseid]
                );
            }
            ...
            $DB->set_field('course_format_options', 'name', 'defaulttileicon',
                ['format' => 'eabctiles', 'name' => 'defaulttiletopleftdisplay', 'courseid' => $newcourseid]);
            $DB->set_field('course_format_options', 'name', 'tileicon',
                ['format' => 'eabctiles', 'name' => 'tiletopleftthistile', 'courseid' => $newcourseid]);
            ...
            $DB->delete_records_select(
                'course_format_options',
                "format  = 'eabctiles' AND name = 'tileicon' AND value = 'course default' AND courseid = :courseid",
                ["courseid" => $newcourseid]
            );
            ...
            $fs = get_file_storage();
            $fs->delete_area_files(context_course::instance($newcourseid)->id, 'format_eabctiles', 'temptilephoto');
        }
    }
    ...
    private static function update_file_records_sections($newcourseid, $contextid, $olditemid, $newitemid) {
        ...
        $fileids = $DB->get_fieldset_select(
            'files', 'id',
            "contextid = :coursecontextid AND component = 'format_eabctiles'
        ...
        if (!empty($fileids)) {
            ...
            foreach ($fileids as $fileid) {
                ...
                if ($oldfile && $oldfile->get_itemid() != $newitemid) {
                    ...
                    \format_eabctiles\local\tile_photo::delete_files_from_ids($newcourseid, $newitemid);
    ...
    /**
     * Process an item for format_eabctiles_tile_options table.
     * @param int $courseid
     * @param int $sectionid
    ...
    private function set_format_opt(int $courseid, int $sectionid, int $optiontype, string $optionvalue) {
        try {
            ...
            throw new moodle_exception(
                'invalidrecordid', 'format_eabctiles', '',
                'Could not insert photo. Database table format_eabctiles_tile_options is not ready.'.
    ...
    private function fail_if_course_includes_excess_sections() {
        $backupinfo = $this->step->get_task()->get_info();
        $maxallowed = \format_eabctiles\local\course_section_manager::get_max_sections();
        ...
        foreach ($backupinfo->sections as $section) {
            ...
            if ($included) {
                ...
                if ($totalincluded > $maxallowed + 1) {
                    ...
                    \core\notification::error(get_string('restoretoomanysections', 'format_eabctiles', $a));
                    throw new moodle_exception('restoretoomanysections', 'format_eabctiles', '', $a);
                }
            }
        }
    }
    ...
    private function check_destination_course_section_count() {
        ...
        $countsections = \format_eabctiles\local\course_section_manager::count_course_sections($courseid);
        if ($countsections > $maxallowed) {
            $a = new stdClass();
            $a->sectionnum = \format_eabctiles\local\course_section_manager::get_max_non_subsection_number($courseid);
            $a->maxallowed = $maxallowed;
            \core\notification::error(get_string('restoreincorrectsections', 'format_eabctiles', $a));
            throw new moodle_exception('restorefailed', 'format_eabctiles', '', $a);
        }
    ...
    private function is_tiles_course(): bool {
        $backupinfo = $this->step->get_task()->get_info();
        return isset($backupinfo->original_course_format) && $backupinfo->original_course_format == 'eabctiles';
    }
}
