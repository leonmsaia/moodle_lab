 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_eabctiles extends core_courseformat\base {
    ...
    public function uses_course_index() {
        return get_config('format_eabctiles', 'usecourseindex');
    }
    ...
    public function get_default_section_name($section) {
        if ($section->section == 0) {
            ...
            return get_string('section0name', 'format_eabctiles');
        } else {
    ...
    public function edit_form_validation($data, $files, $errors) {
        ...
        if (!$data['enablecompletion'] && $data['courseshowtileprogress']) {
            $reterrors['courseshowtileprogress'] = get_string('courseshowtileprogress_error', 'format_eabctiles');
        }
        if (($data['displayfilterbar'] == \format_eabctiles\local\format_option::FILTER_OUTCOMES_ONLY
                || $data['displayfilterbar'] == \format_eabctiles\local\format_option::FILTER_OUTCOMES_AND_NUMBERS)
            && empty(\format_eabctiles\local\filters::get_course_outcomes($courseid))) {
            $outcomeslink = html_writer::link(
                new moodle_url('/grade/edit/outcome/course.php', ['id' => $courseid]),
                new lang_string('outcomes', 'format_eabctiles')
            );
            $reterrors['displayfilterbar'] = get_string('displayfilterbar_error', 'format_eabctiles') . ' ' . $outcomeslink;
        }
        return $reterrors;
    }
    ...
    public function extend_course_navigation($navigation, navigation_node $node) {
        ...
        if ($PAGE->user_is_editing() && has_capability('moodle/site:config', context_system::instance())) {
            $settingsnode = navigation_node::create(
                get_string('resetalltiles', 'format_eabctiles'),
                new moodle_url('/course/format/eabctiles/editor/reset.php', ['courseid' => $course->id]),
    ...
    public function get_view_url($section, $options = []) {
        ...
        } else if (\format_eabctiles\local\util::using_js_nav()) {
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            $courseformatoptions = [
                ...
                'basecolour' => [
                    'default' => get_config('format_eabctiles', 'tilecolour1'),
                    'type' => PARAM_TEXT,
                ],
            ...
            if ((get_config('format_eabctiles', 'followthemecolour'))) {
        ...
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            $tilespalette = \format_eabctiles\local\util::get_tiles_palette();
            $tileicons = (new \format_eabctiles\local\icon_set)->available_tile_icons($this->get_courseid());
            ...
            $label = get_string('defaulttileicon', 'format_eabctiles');
            $courseformatoptionsedit['defaulttileicon'] = [
                'label' => $label,
                'element_type' => 'select',
                'element_attributes' => [$tileicons],
                'help' => 'defaulttileicon',
                'help_component' => 'format_eabctiles',
            ];
            if (!(get_config('format_eabctiles', 'followthemecolour'))) {
                $courseformatoptionsedit['basecolour'] = [
                    'label' => new lang_string('basecolour', 'format_eabctiles'),
                    'element_type' => 'select',
            ...
            $attributes = [
                \format_eabctiles\local\format_option::FILTER_NONE => new lang_string('hide', 'format_eabctiles'),
                \format_eabctiles\local\format_option::FILTER_NUMBERS_ONLY => new lang_string('filternumbers', 'format_eabctiles'),
            ];
            $outcomeslink = '(' . new lang_string('outcomesunavailable', 'format_eabctiles') . ')';
            global $CFG;
            if (!empty($CFG->enableoutcomes)) {
                ...
                $attributes[\format_eabctiles\local\format_option::FILTER_OUTCOMES_ONLY]
                    = new lang_string('filteroutcomes', 'format_eabctiles');
                $attributes[\format_eabctiles\local\format_option::FILTER_OUTCOMES_AND_NUMBERS]
                    = new lang_string('filterboth', 'format_eabctiles');
            }
            $courseformatoptionsedit['displayfilterbar'] = [
                'label' => new lang_string('displayfilterbar', 'format_eabctiles') . ' ' . $outcomeslink,
                'element_type' => 'select',
            ...
            $courseformatoptionsedit['courseshowtileprogress'] = [
                'label' => new lang_string('courseshowtileprogress', 'format_eabctiles'),
                'element_type' => 'select',
                'element_attributes' => [
                    [
                        0 => new lang_string('hide', 'format_eabctiles'),
                        1 => new lang_string('asfraction', 'format_eabctiles'),
                        2 => new lang_string('aspercentagedial', 'format_eabctiles'),
                    ],
                ],
            ...
            $allowsubtilesview = get_config('format_eabctiles', 'allowsubtilesview');
            if ($allowsubtilesview) {
                $courseformatoptionsedit['courseusesubtiles'] = [
                    'label' => new lang_string('courseusesubtiles', 'format_eabctiles'),
            ...
            $courseformatoptionsedit['courseusebarforheadings'] = [
                'label' => new lang_string('courseusebarforheadings', 'format_eabctiles'),
                'element_type' => 'advcheckbox',
                'element_attributes' => [get_string('yes')],
            ...
            if ($allowsubtilesview) {
                $courseformatoptionsedit['usesubtilesseczero'] = [
                    'label' => new lang_string('usesubtilesseczero', 'format_eabctiles'),
                    'element_type' => 'advcheckbox',
                    'element_attributes' => [get_string('notrecommended', 'format_eabctiles')],
    ...
    public function section_format_options($foreditform = false) {
        ...
        $usingoutcomesfilter = in_array(
            ...
            [
                \format_eabctiles\local\format_option::FILTER_OUTCOMES_ONLY,
                \format_eabctiles\local\format_option::FILTER_OUTCOMES_AND_NUMBERS,
            ]
        );
        ...
        if ($foreditform) {
            ...
            if ($usingoutcomesfilter) {
                ...
                $label = get_string('tileoutcome', 'format_eabctiles') . ' ' . $outcomeslink;
                $outcomes = \format_eabctiles\local\filters::get_course_outcomes($course->id);
                if (!empty($outcomes)) {
                    $outcomes[0] = get_string('none', 'format_eabctiles');
                }
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        if (!$forsection) {
            $jsparams = [
                ...
                'courseId' => $COURSE->id,
                get_config('format_eabctiles', 'allowphototiles'),
                get_config('format_eabctiles', 'documentationurl'),
            ];
            $PAGE->requires->js_call_amd('format_eabctiles/edit_form_helper', 'init', $jsparams);
        } else {
            // Add a tip to the edit section form for anyone who does not know how to edit tile icon/photos.
            $mform->addElement('html',
                html_writer::div(
                    html_writer::div(get_string('setbackgroundphoto', 'format_eabctiles'), 'col-md-3 col-form-label')
                    . html_writer::div(get_string('eabctileselecttip', 'format_eabctiles'), 'col-md-9'),
                    'row mb-3'
                )
            );
        }
     ...
     *
     * If course format was changed to 'eabctiles', we try to copy options
                ...
                if ($oldcourse !== null && $oldcourse['format'] == 'grid') {
                    ...
                    if ($dbman->table_exists($gridtable)) {
                        ...
                        foreach ($gridformaticons as $gridformaticon) {
                            ...
                            $tilephoto = new \format_eabctiles\local\tile_photo($coursecontext, $gridformaticon->sectionid);
                            ...
                            if ($gridfile && !$existingtilephoto) {
                                ...
                                $fs = get_file_storage();
                                $newfilerecord = \format_eabctiles\local\tile_photo::file_api_params();
    ...
    public function update_section_format_options($data) {
        ...
        $oldvalues = [
            'outcomethistile' => $DB->get_record(
                ...
                ['courseid' => $this->courseid, 'format' => 'eabctiles', 'sectionid' => $data['id'], 'name' => 'tileoutcomeid']
            ),
        ];

        // If the edit is taking place from format_eabctiles_inplace_editable(),
        ...
        foreach ($keystoremove as $key) {
            if (!isset($data[$key])) {
                $DB->delete_records(
                    'course_format_options',
                    ['courseid' => $this->courseid, 'format' => 'eabctiles', 'sectionid' => $data['id'], 'name' => $key]
                );
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = false,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_eabctiles');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_eabctiles', $title);
        }
    ...
    public function section_action($section, $action, $sr) {
        ...
        if ($section->section && ($action === 'setmarker' || $action === 'removemarker')) {
            // Format 'eabctiles' allows to set and remove markers in addition to common section actions.
        ...
        $renderer = $PAGE->get_renderer('format_eabctiles');
    ...
    public function page_set_course(moodle_page $page) {
        global $SESSION, $DB;
        $tilesactionparam = optional_param('format-eabctiles-action', '', PARAM_TEXT);
        if ($tilesactionparam) {
            require_sesskey();
        }
        if ($tilesactionparam == 'toggleanimatednav') {
            if (get_config('format_eabctiles', 'usejavascriptnav')) {
                // User is toggling JS nav setting.
                $userpreferencenamejsnav = 'format_eabctiles_stopjsnav';
            ...
            if (get_config('format_eabctiles', 'highcontrastmodeallow')) {
                ...
                $userpreferencenamecontrast = 'format_eabctiles_high_contrast_mode';
        ...
        if ($page->state <= $page::STATE_BEFORE_HEADER) {
            ...
            if ($page->pagetype == 'course-view') {
                if ((optional_param('section', 0, PARAM_INT)
                    ...
                    && !\format_eabctiles\local\util::using_js_nav()) {
                    $courseusessubtiles = get_config('format_eabctiles', 'allowsubtilesview')
                        && ($page->course->id ?? null)
                        && $DB->get_field(
                            'course_format_options', 'value',
                            ['courseid' => $page->course->id, 'format' => 'eabctiles', 'sectionid' => 0, 'name' => 'courseusesubtiles']
                        ) == "1";
                    if (!$courseusessubtiles) {
                        $page->add_body_class("format-eabctiles-single-sec");
                    }
                }
                if (\format_eabctiles\local\util::using_high_contrast()) {
                    $page->add_body_class("format-eabctiles-high-contrast");
                }
            }
        }
    }
 ...
 * @throws dml_exception
 */
function format_eabctiles_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            "SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?",
            [$itemid, 'eabctiles'], MUST_EXIST);
 ...
 */
function format_eabctiles_get_fontawesome_icon_map() {
    $iconset = new format_eabctiles\local\icon_set();
 ...
 * @return void
 */
function format_eabctiles_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    ...
    $fileapiparams = \format_eabctiles\local\tile_photo::file_api_params();
    $fs = get_file_storage();
 ...
 */
function format_eabctiles_output_fragment_get_cm_list(array $args): string {
    global $PAGE, $DB;
    $sectionid = $args['sectionid'];
    $section = $DB->get_record('course_sections', ['id' => $sectionid]);
    if (!$section) {
        throw new moodle_exception(
            'invalidsectionid', 'format_eabctiles', '', $sectionid, "Section ID '$sectionid' not found"
        );
    }
    $course = get_course($section->course);

    if ($course->format != 'eabctiles') {
        throw new moodle_exception(
            'invalidsectionid', 'format_eabctiles', '',
    ...
    $renderer = $PAGE->get_renderer('format_eabctiles');
    $templateable = new \format_eabctiles\output\course_output($course, true, $section->section);
    $data = $templateable->export_for_template($renderer);

    $template = $section->section == 0 ? 'format_eabctiles/section_zero' : 'format_eabctiles/single_section_content';
 ...
 * @return string The HTML to add to page.
 */
function format_eabctiles_output_fragment_get_cm_content(array $args): string {
    global $DB, $CFG, $PAGE;
    ...
    if ($mod) {
        $allowedmodules = explode(",", get_config('format_eabctiles', 'modalmodules'));
        ...
        if ($mod->modname == 'page') {
            ...
            return \format_eabctiles\local\util::format_cm_content_text($mod->modname, $record, $modcontext);
        }
        if ($treataslabel) {
