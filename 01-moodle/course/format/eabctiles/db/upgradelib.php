...
/**
 *
 * This method finds all courses in 'eabctiles' format that have actual number of sections
 * bigger than their 'numsections' course format option.
 * For each such course we call {@see format_eabctiles_upgrade_hide_extra_sections()} and
 * either delete or hide "orphaned" sections.
 * @throws dml_exception
 * @throws coding_exception
 */
function format_eabctiles_upgrade_remove_numsections() {
    global $DB;
    ...
    $params = ['format1' => 'eabctiles', 'format2' => 'eabctiles', 'numsections' => 'numsections'];
    ...
    foreach ($needfixing as $courseid => $numsections) {
        format_eabctiles_upgrade_hide_extra_sections($courseid, $numsections);
    }

    $DB->delete_records('course_format_options', ['format' => 'eabctiles', 'sectionid' => 0, 'name' => 'numsections']);
}
 ...
 * @throws coding_exception
 */
function format_eabctiles_upgrade_hide_extra_sections($courseid, $numsections) {
    global $DB;
 ...
 * @throws dml_exception
 */
function format_eabctiles_remove_unused_format_options() {
    global $DB;
    $DB->delete_records('course_format_options', ['format' => 'eabctiles', 'name' => 'showachladderbutton']);
    $DB->delete_records('course_format_options', ['format' => 'eabctiles', 'name' => 'prefixtitlewithnumber']);
    $DB->delete_records('course_format_options', ['format' => 'eabctiles', 'name' => 'showgradesbutton']);
}
