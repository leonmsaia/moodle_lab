...
/**
 * Load the format_eabctiles JavaScript for the course edit settings page /course/edit.php?id=xxx
 *
 * @module      format_eabctiles/completion
    ...
    function ($, Templates, config, ajax, str, coreManualCompletion) {
        ...
        const setProgressIndicator = function(sectionNumber, numComplete, numOutOf) {
            ...
            if (tileProgressIndicator.length) {
                ...
                if (sectionNumber === 0) {
                    const overallProgressOuter = $('#eabctiles-overall-progress-outer');
                    if (percent === 100) {
        ...
        const triggerCompletionChangedEvent = function (sectionNum, cmId) {
            if (sectionNum > 0 || cmId > 0) {
                ...
                $(document).trigger('format-eabctiles-completion-changed', data);
            }
        };

        /**
         * If we have called format_eabctiles_get_section_information then we need to add the result to the DOM.
                ...
                if (sec.sectionnum > 0) {
                    ...
                    if (availabilityInfoDiv.length > 0 && sec.isavailable && !sec.availabilitymessage) {
                        ...
                        if (availabilityInfoDiv.length > 0) {
                            ...
                            Templates.render("format_eabctiles/availability_info", {
        ...
        return {
            init: function (courseIdInit) {
                ...
                $(document).ready(function () {
                    var loadingString = '...';
                    str.get_strings([{key: "loading", component: "format_eabctiles"}]).done(function (s) {
                        loadingString = s[0] + '  ...';
                    });
