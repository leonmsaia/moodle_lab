 ...
 * if the user picks an invalid option it will be detected by format_eabctiles::edit_form_validation (lib.php)
 * but this is to help them avoid triggering that if they have JS enabled
 *
 * @module      format_eabctiles/edit_form_helper
        ...
        return {
            init: function (pageType, courseDefaultIcon, courseId, allowphototiles, documentationUrl) {
                $(document).ready(function () {
                        ...
                        if (e.currentTarget.value !== "0") {
                            ...
                            if (enableCompBox.val() === "0") {
                                ...
                                str.get_strings([
                                    ...
                                    {key: "completionswitchhelp", component: "format_eabctiles"}
                                ]).done(function (s) {
                                    Notification.alert(
                                        s[0],
                                        s[1]
                    ...
                    Templates.render("format_eabctiles/colour_picker", {
                        ...
                        // This comes from core.  We want body to have format-eabctiles class for our colour picker CSS, so we add it.
                        var body = $("body");
                        if (!body.hasClass("format-eabctiles")) {
                            body.addClass("format-eabctiles");
                        }
                    });
                    ...
                    if (pageType === "course-edit") {
                        ...
                        Templates.render("format_eabctiles/icon_picker_launch_btn", {
                            ...
                            selectBox.hide();
                            require(["format_eabctiles/edit_icon_picker"], function(iconPicker) {
                    ...
                    Templates
                        .render("format_eabctiles/edit_form_helptext", {documentationurl: documentationUrl + 'teachers'})
                        .done(function (html) {
