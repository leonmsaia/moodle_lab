 ...
 */
namespace format_eabctiles\privacy;

use core_privacy\local\metadata\collection;

/**
 * Privacy Subsystem implementation for format_eabctiles
 *
 * @copyright  2018 David Watson {@link http://evolutioncode.uk}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class provider implements \core_privacy\local\metadata\provider,
    ...
    public static function export_user_preferences(int $userid) {
        foreach (self::get_prefs_and_defaults() as $prefname => $default) {
            ...
            if (isset($preference)) {
                ...
                \core_privacy\local\request\writer::export_user_preference('format_eabctiles', $prefname,
                    $value, get_string("privacy:metadata:preference:$prefname", 'format_eabctiles'));
            }
        }
    ...
    private static function get_prefs_and_defaults(): array {
        return [
            'format_eabctiles_stopjsnav' => 0,
            'format_eabctiles_high_contrast_mode' => 0,
        ];
    }
}
