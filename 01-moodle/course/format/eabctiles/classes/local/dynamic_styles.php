...
class dynamic_styles {
    ...
    public static function get_tiles_dynamic_css(): string {
        ...
        if ($idparam) {
            ...
            if ($courseid) {
                ...
                return $m->render(
                    file_get_contents("$CFG->dirroot/course/format/eabctiles/templates/dynamic_styles.mustache"),
                    $data
                );
            }
        }
     ...
     * @see \format_eabctiles\local\util::width_template_data()
     * @param int $courseid
     * @return array
     * @throws \dml_exception
     * @throws \moodle_exception
    ...
    public static function data_for_template(int $courseid): array {
        ...
        $tilestyle = get_config('format_eabctiles', 'eabctilestyle') ?? \format_eabctiles\output\course_output::TILE_STYLE_STANDARD;
        $usinghighcontrast = \format_eabctiles\local\util::using_high_contrast();
        ...
        $outputdata = [
            ...
            'pluginconfigcss' => trim(get_config('format_eabctiles', 'customcss') ?? ''),
        ];
        ...
        if (get_config('format_eabctiles', 'allowphototiles')) {
            ...
            $outputdata['photo_tile_text_bg_opacity'] = $usinghighcontrast ? 1 :
                1.0 - (float)get_config('format_eabctiles', 'phototiletitletransarency');
            ...
            $outputdata['phototiletextpadding'] = number_format(
                (float)get_config('format_eabctiles', 'phototitletitlepadding') / 10, 1
            );
            $outputdata['phototiletextlineheight'] = number_format(
                (float)get_config('format_eabctiles', 'phototitletitlelineheight') / 10, 1
            );
        }

        // Tile fitter if used.
        $outputdata['usingtilefitter'] = \format_eabctiles\local\util::using_tile_fitter();
        $outputdata['tilefittermaxwidth'] = $outputdata['usingtilefitter']
            ? \format_eabctiles\local\util::get_tile_fitter_max_width($courseid) : 0;
    ...
    public static function get_tile_base_colour($coursebasecolour): string {
        ...
        if (!(get_config('format_eabctiles', 'followthemecolour'))) {
            if (!$coursebasecolour) {
                ...
                $result = get_config('format_eabctiles', 'tilecolour1');
            } else {
        ...
        if (!$result || !preg_match($hexpattern, $result)) {
            ...
            $result = get_config('format_eabctiles', 'tilecolour1') ?? self::DEFAULT_COLOUR;
        }
        return $result;
    }
    ...
    public static function page_needs_loading_icon(int $courseid): bool {
        if (!\format_eabctiles\local\util::using_tile_fitter()) {
            return false;
        }
        return !\format_eabctiles\local\util::get_tile_fitter_max_width($courseid);
    }
}
