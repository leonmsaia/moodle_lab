...
/**
 * format_eabctiles settings tabs
 *
...
class admin_settingspage_tabs extends \admin_settingpage {
    ...
    public function output_html() {
        ...
        foreach ($this->get_tabs() as $index => $tab) {
            $data = [
                'name' => str_replace('format_eabctiles/' , '', $tab->name),
            ...
            if ($tab->name === "format_eabctiles/tab-colours") {
                $data['iscolourstab'] = 1;
            }

            $context['tabs'][] = $data;
        }
        $context['documentationurl'] = get_config('format_eabctiles', 'documentationurl');
        $context['sesskey'] = sesskey();

        return $OUTPUT->render_from_template('format_eabctiles/admin_setting_tabs', $context);
    }
}
