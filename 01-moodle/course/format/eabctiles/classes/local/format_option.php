...
namespace format_eabctiles\local;
...
class format_option {
    ...
    /**
     * When we store the photo in db format_eabctiles_tile_options table, specify if it's a section or cm photo.
     */
    ...
    /**
     * Get a course format option full record from the format_eabctiles_tile_options table.
     * @param int $courseid
    ...
    public static function get_db_record(int $courseid, int $optiontype, int $elementid) {
        ...
        try {
            ...
            return $DB->get_record(
                'format_eabctiles_tile_options',
                ['courseid' => $courseid, 'optiontype' => $optiontype, 'elementid' => $elementid]
            );
        } catch (\Exception $e) {
            debugging('Could not get record from db format_eabctiles_tile_options table ' . $e->getMessage(), DEBUG_DEVELOPER);
            return false;
        }
    }

    /**
     * Get a course format option value from the format_eabctiles_tile_options table.
     * @param int $courseid
     * @param int $optiontype
    ...
    public static function get_multiple(int $courseid, int $optiontype): array {
        ...
        return $DB->get_records_sql(
            "SELECT elementid, optionvalue
                FROM {format_eabctiles_tile_options} WHERE courseid = ? AND optiontype = ?",
            [$courseid, $optiontype]
        );
    }

    /**
     * Unset a course format option value in the format_eabctiles_tile_options table.
     * @param int $courseid
     * @param int $optiontype
     * @param int $elementid
     * @return bool
    ...
    public static function unset(int $courseid, int $optiontype, int $elementid) {
        ...
        try {
            $record = self::get_db_record($courseid, $optiontype, $elementid);
            if ($record) {
                return $DB->delete_records('format_eabctiles_tile_options', ['id' => $record->id]);
            }
        } catch (\Exception $e) {
    ...
    public static function unset_multiple_types(int $courseid, int $elementid, array $optiontypes): bool {
        ...
        foreach ($optiontypes as $optiontype) {
            ...
            if ($DB->delete_records('format_eabctiles_tile_options', $params)) {
                $result = true;
            }
        }
        return $result;
    ...
    /**
     * Unset all course format option values in the format_eabctiles_tile_options table for a course.
     * @param int $courseid
     * @return bool
     */
    public static function unset_all_course(int $courseid): bool {
        global $DB;
        try {
            $result = $DB->delete_records('format_eabctiles_tile_options', ['courseid' => $courseid]);

            // Delete legacy tile icon choices.
            return $result && $DB->delete_records_select(
                'course_format_options',
                "courseid = :courseid AND format = 'eabctiles' AND name IN('tilephoto', 'tileicon')",
                ['courseid' => $courseid]
            );
        } catch (\Exception $e) {
            debugging('Could not unset records ' . $e->getMessage(), DEBUG_DEVELOPER);
    ...
    /**
     * Set a course format option in the format_eabctiles_tile_options table.
     * @param int $courseid
     * @param int $optiontype
     * @param int $elementid
    ...
    public static function set(int $courseid, int $optiontype, int $elementid, string $optionvalue): bool {
        ...
        if ($optionvalue) {
            ...
            return $DB->insert_record('format_eabctiles_tile_options', $record);
        }
        return true;
    }
    /**
     * Validate an option type for format_eabctiles_tile_options table.
     * @param int $optiontype
     * @return bool
     * @throws \moodle_exception
     */
    public static function validate_option_type(int $optiontype) {
        ...
        throw new \moodle_exception('invalidargument', 'format_eabctiles', '', '', 'Unexpected option type ' . $optiontype);
    }
    ...
    public static function migrate_legacy_format_option_type(int $courseid, int $optiontype, int $sectionid = 0) {
        ...
        // If we have any legacy course format options which need moving to format_eabctiles_tile_options table, move them.
    ...
    public static function get_legacy_format_options_recordset(
        ...
        $queryparams = ['format' => 'eabctiles', 'courseid' => $courseid];
     ...
     * @see \format_eabctiles\task\migrate_legacy_data
     * @param int $courseid
     * @return bool
     * @throws \dml_exception
     */
    public static function needs_migration_incomplete_warning(int $courseid): bool {
        ...
        $haslegacyoptions = $DB->record_exists_sql(
            "SELECT id FROM {course_format_options}
                WHERE courseid = ? and format = 'eabctiles' AND name IN ('tilephoto', 'tileicon')",
            [$courseid]
        );
        if (!$haslegacyoptions) {
            return false;
        ...
        $hasmigratedoptions = $DB->record_exists_sql(
            "SELECT id FROM {format_eabctiles_tile_options} WHERE courseid = ? AND optiontype IN (?, ?)",
        ...
        if ($legacyoptions->valid()) {
            foreach ($legacyoptions as $legacyoption) {
                ...
                $newrecordexists = $DB->record_exists_sql(
                    "SELECT id FROM {format_eabctiles_tile_options}
