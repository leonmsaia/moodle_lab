...
/**
 * General utilities class for format_eabctiles.
...
class util {
    ...
    public static function get_course_mod_info(int $courseid, int $cmid): ?object {
        ...
        $modaltype = \format_eabctiles\local\modal_helper::cm_modal_type($courseid, $cmrecord->id);
    ...
    public static function get_tiles_plugin_release(): string {
        ...
        $plugin = new \stdClass();
        $plugin->release = '';
        require("$CFG->dirroot/course/format/eabctiles/version.php");
    ...
    public static function using_js_nav() {
        ...
        return get_config('format_eabctiles', 'usejavascriptnav')
            && !get_user_preferences('format_eabctiles_stopjsnav', 0)
    ...
    public static function using_tile_fitter(): bool {
        ...
        if (optional_param('skipcheck', 0, PARAM_INT)) {
            ...
            $SESSION->format_eabctiles_skip_width_check = 1;
            return false;
        }

        return get_config('format_eabctiles', 'fittilestowidth')
            && \core_useragent::get_device_type() != \core_useragent::DEVICETYPE_MOBILE
            && ($SESSION->format_eabctiles_skip_width_check ?? null) != 1;
    }
    ...
    public static function get_tile_fitter_max_width(int $courseid): int {
        ...
        $var = 'format_eabctiles_width_' . $courseid;
    ...
    public static function get_tiles_palette() {
        $palette = [];
        for ($i = 1; $i <= 10; $i++) {
            $colourname = get_config('format_eabctiles', 'colourname' . $i);
            $tilecolour = get_config('format_eabctiles', 'tilecolour' . $i);
    ...
    public static function init_js($course, int $contextid, ?int $sectionnumber) {
        ...
        if (!$PAGE->user_is_editing()) {
            ...
            $jsparams = [
                ...
                'assumeDataStoreContent' => get_config('format_eabctiles', 'assumedatastoreconsent'),
                'reOpenLastSection' => get_config('format_eabctiles', 'reopenlastsection'),
                'userId' => $USER->id,
                'fitTilesToWidth' => self::using_tile_fitter(),
                'enablecompletion' => $course->enablecompletion,
                'usesubtiles' => get_config('format_eabctiles', 'allowsubtilesview') && $course->courseusesubtiles,
                'courseContextId' => $contextid,
            ];
            $PAGE->requires->js_call_amd('format_eabctiles/course', 'init', $jsparams);
        } else {
            // Initialise JS for when editing mode is on.
            $editparams = [
                'courseId' => $course->id,
                'pageType' => $PAGE->pagetype,
                'allowPhotoTiles' => get_config('format_eabctiles', 'allowphototiles'),
                'documentationurl' => get_config('format_eabctiles', 'documentationurl'),
                'maxnumbericons' => self::get_icon_picker_max_number_icons(),
            ];
            $PAGE->requires->js_call_amd('format_eabctiles/edit_icon_picker', 'init', $editparams);
        }
        if ($course->enablecompletion) {
            $PAGE->requires->js_call_amd('format_eabctiles/completion', 'init', [$course->id]);
        }
    }
    ...
    public static function get_js_config_data(int $courseid) {
        ...
        if (get_config('format_eabctiles', 'usecourseindex')) {
            $allowedmodals = \format_eabctiles\local\modal_helper::allowed_modal_modules();
            ...
            $jsconfigvalues['modalAllowedCmids'] = json_encode(
                \format_eabctiles\local\modal_helper::get_modal_allowed_cm_ids_integer_list($courseid, true)
            );
        }

        $jsconfigvalues['defaultcourseicon'] = $DB->get_field(
            'course_format_options', 'value',
            ['courseid' => $courseid, 'format' => 'eabctiles', 'sectionid' => 0, 'name' => 'defaulttileicon']
        );
    ...
    public static function using_high_contrast(): bool {
        return get_config('format_eabctiles', 'highcontrastmodeallow')
            && get_user_preferences('format_eabctiles_high_contrast_mode', 0);
    }
