...
class icon_set {
    ...
    public function available_tile_icons($courseid = 0) {
        ...
        if ($this->supports_font_awesome()) {
            foreach ($this->fontawesometileicons as $iconname) {
                ...
                if ($stringmanager->string_exists('icontitle-' . $pixname, 'format_eabctiles')) {
                    $displayname = get_string('icontitle-' . $pixname, 'format_eabctiles');
                } else {
        ...
        $iconsindirectory = get_directory_list($CFG->dirroot
            . '/course/format/eabctiles/pix/tileicon', '', false, false, true);
        foreach ($iconsindirectory as $icon) {
            ...
            if (!isset($availableicons[$filename])) {
                if ($stringmanager->string_exists('icontitle-' . $filename, 'format_eabctiles')) {
                    $displayname = get_string('icontitle-' . $filename, 'format_eabctiles');
                } else {
     ...
     * @see format_eabctiles_get_fontawesome_icon_map()
     * @return array
     */
    public function get_font_awesome_icon_map() {
        ...
        $generalicons = [
            'format_eabctiles:camera' => 'fa-camera',
            'format_eabctiles:check' => 'fa-check',
            'format_eabctiles:chevron-left' => 'fa-chevron-left',
            'format_eabctiles:chevron-right' => 'fa-chevron-right',
            'format_eabctiles:clone' => 'fa-clone',
            'format_eabctiles:close' => 'fa-close',
            'format_eabctiles:cloud-download' => 'fa-cloud-download',
            'format_eabctiles:cloud-upload' => 'fa-cloud-upload',
            'format_eabctiles:completion-check' => 'fa-check',
            'format_eabctiles:completion-fail' => 'fa-times',
            'format_eabctiles:eye-slash' => 'fa-eye-slash',
            'format_eabctiles:filter' => 'fa-filter',
            'format_eabctiles:home' => 'fa-home',
            'format_eabctiles:image' => 'fa-image',
            'format_eabctiles:lock' => 'fa-lock',
            'format_eabctiles:pencil' => 'fa-pencil',
            'format_eabctiles:random' => 'fa-random',
            'format_eabctiles:sort' => 'fa-sort',
            'format_eabctiles:toggle-off' => 'fa-toggle-off',
            'format_eabctiles:toggle-on' => 'fa-toggle-on',
        ];

        $tileicons = [];
        foreach ($this->fontawesometileicons as $icon) {
            $pixname = str_replace('fa-', '', $icon);
            $tileicons['format_eabctiles:tileicon/' . $pixname] = $icon;
        }
