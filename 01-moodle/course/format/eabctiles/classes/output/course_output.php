 ...
 */
namespace format_eabctiles\output;

use format_eabctiles\local\format_option;
use format_eabctiles\local\tile_photo;
use format_eabctiles\local\filters;
use format_eabctiles\local\util;
...
class course_output implements \renderable, \templatable {
    ...
    public function __construct($course, $fromajax = false, $sectionnum = null, \renderer_base|null $courserenderer = null) {
        ...
        $this->modalscmids = \format_eabctiles\local\modal_helper::get_modal_allowed_cm_ids_integer_list(
        ...
        $this->usingjsnav = \format_eabctiles\local\util::using_js_nav();
    }
    ...
    private function get_basic_data() {
        ...
        $data['showJScancelLink'] = isset($SESSION->format_eabctiles_jssuccessfullyused) ? 1 : 0;
        $data['editing'] = $this->isediting;
        $data['sesskey'] = sesskey();
        $data['showinitialpageloadingicon'] = !$this->isediting
            && \format_eabctiles\local\dynamic_styles::page_needs_loading_icon($this->course->id);
        $data['jsnavadminallowed'] = get_config('format_eabctiles', 'usejavascriptnav');
        $data['usingjsnav'] = $this->usingjsnav ? 1 : 0;
        $data['jsnavuserenabled'] = !get_user_preferences('format_eabctiles_stopjsnav');
        ...
        $data['useSubtiles'] = get_config('format_eabctiles', 'allowsubtilesview') && $this->courseformatoptions['courseusesubtiles'];
        $data['usetooltips'] = get_config('format_eabctiles', 'usetooltips');
        $data['outerextraclasses'] = get_config('format_eabctiles', 'subtileiconcolourbackground')
            ? 'format-eabctiles-colour-subtile-icon-bg' : '';
        ...
        if ($data['canedit'] && format_option::needs_migration_incomplete_warning($this->course->id)) {
            $message = get_string('coursephotomigrationincomplete', 'format_eabctiles');
            $message .= \html_writer::link(
                new \moodle_url('/course/format/eabctiles/editor/migratecoursedata.php', ['courseid' => $this->course->id]),
                '<i class="icon fa fa-cog me-1"></i>' . get_string('fixproblems', 'format_eabctiles'),
                ['class' => 'ms-1']
            );
            $data['editorwarnings'][] = [
                'text' => $message,
    ...
    private function append_section_zero_data($data, $output) {
        ...
        $dias =  ($COURSE->enddate!=0) ? abs($COURSE->enddate - $COURSE->startdate) / 86400 : 1;
        ...
        $data['section_zero']['courseid'] = $COURSE->id;
        $data['section_zero']['userid'] = $USER->id;
        $data['section_zero']['iselearning'] = $iselearning;
        $data['section_zero']['closeactivity'] = "";
        
        //$data['section_zero']['summary'] = self::temp_format_summary_text($seczero);
        $data['section_zero']['summary'] = $COURSE->summary;
        $data['section_zero']['duration'] = $dias;
        ...
        $data['section_zero']['is_course_streaming'] = \format_eabctiles\utils\eabctiles_utils::is_course_streaming($COURSE->id);

        // Only show section zero if we need it.
        $data['section_zero_show'] = 0;
        if ($this->sectionnum == 0 || get_config('format_eabctiles', 'showseczerocoursewide')) {
        ...
        if (has_capability('format/eabctiles:closegroup',  \context_course::instance($COURSE->id), $USER->id)) {
            
            $groups = groups_get_all_groups($COURSE->id,$USER->id);
            $suspend = empty($groups) ? false : true;
            
            foreach($groups as $group){
                $groupsuspend = $DB->get_record('format_eabctiles_suspendgrou',array('courseid'=>$COURSE->id, 'groupid'=>$group->id));                
                if (empty($groupsuspend)){
                    $suspend = false;
                break;
                }
            }
            ...
            if ($suspend){
                ...
                $data['section_zero']['closeactivity'] = $output->single_button(new \moodle_url('/course/format/eabctiles/closeactivity.php', array('id' => $COURSE->id)), get_string('endorsuspendactivity', 'format_eabctiles'), 'get');
            }
        }
        $sesiones =  $this->get_session_user($COURSE->id, $USER->id);
        $data['section_sessions']['session'] = !empty($sesiones) ? array('sesiones' => $sesiones) : false;
        $data['section_sessions']['is_course_streaming'] = \format_eabctiles\utils\eabctiles_utils::is_course_streaming($COURSE->id);

        return $data;
    }

    private function get_session_user($courseid, $userid){
        global $DB, $OUTPUT;

        $courses = $DB->get_records('eabcattendance', array('course'=>$courseid ));
        $idcourses = [];
        foreach($courses as $course){
            array_push($idcourses, $course->id);
        }
        $sesiones = [];
        $str = implode (",", $idcourses);
        if ($str){
            $sql = "SELECT * FROM {eabcattendance_sessions} WHERE eabcattendanceid in ($str)";
            $params = array();
            ...
            if (!empty($results)) {
                ...
                foreach($results as $result){
                    array_push($idsessions, $result->id);
                }
        
                $strSes = implode (",", $idsessions);
                $params = array();
        
                $is_streamin = \format_eabctiles\utils\eabctiles_utils::is_course_streaming($courseid);

                if($is_streamin == false){
                    //si no es curso streming muestro las sesiones en las que el usuario tiene nota
                    $sql = 'SELECT l.sessionid,l.statusid, s.sessdate as finicio, s.duration as duracion, e.description 
                    FROM {eabcattendance_log} l 
                    JOIN {eabcattendance_sessions} s on l.sessionid = s.id 
                    JOIN {eabcattendance_statuses} e on l.statusid = e.id 
                    WHERE studentid = '.$userid.' and sessionid in ('.$strSes.')';
            
                    $resultsSes = $DB->get_records_sql($sql, $params);   
                    
                    foreach($resultsSes as $sesion){
                        $info = array('duracion' => intval($sesion->duracion/60), 'finicio' => userdate($sesion->finicio, get_string('strftimedatetimeshort', 'core_langconfig')), 'estatus' => $sesion->description);
                        array_push($sesiones,$info);
                    } 
                } else {
                    //si es curso streming muestro las sesiones en las que el usuario eeste y la url del mismo
                    $grouparray = array();
                    ...
                    if(!empty($results_groups)){
                        ...
                        $sql = 'select *, s.sessdate as finicio, s.duration as duracion
                        FROM {eabcattendance_sessions}  as s
                        WHERE s.groupid IN (' . $str_group . ')';
                        $results_sesions = $DB->get_records_sql($sql);
                        if(!empty($results_sesions)){
                            foreach($results_sesions as $sesion){
                                $urlstreaming_flag = false;
                                $urlseminarioweb = '';
                                $sesion_back = $DB->get_record('sesion_back', array('id_sesion_moodle' => $sesion->id));
                                if (!empty($sesion_back)) {
                                    $terminocapacitacion = \format_eabctiles\utils\eabctiles_utils::is_valid_date_link($sesion);
                                    if (!empty($sesion_back->urlseminarioweb)) {
                                        $urlstreaming_flag = true;
                                    }
                                }
                                if($urlstreaming_flag == true && $terminocapacitacion == true){
                                    $img = \html_writer::img($OUTPUT->image_url("icon", "format_eabctiles"), '', array('title' => '', 'style' => 'width: 30px'));
                                    $urlseminarioweb = \html_writer::link($sesion_back->urlseminarioweb, $img, array("target" => "_blank"));
                                    $info = array('duracion' => intval($sesion->duration/60), 'finicio' => userdate($sesion->finicio, get_string('strftimedatetimeshort', 'core_langconfig')), 'urlseminarioweb' => $urlseminarioweb);
                                    array_push($sesiones,$info);
                                }
                            }
                        }
    ...
    private function get_course_format_options($fromajax) {
        ...
        if (!get_config('format_eabctiles', 'allowsubtilesview')) {
    ...
    private function append_single_section_page_data($output, $data) {
        ...
        if (get_config('format_eabctiles', 'enablelinebreakfilter')) {
        ...
        if (get_config('format_eabctiles', 'allowphototiles')) {
            ...
            if ($tilephotourl) {
                ...
                $data['phototileediturl'] = new \moodle_url(
                    '/course/format/eabctiles/editor/editimage.php',
    ...
    private function append_multi_section_page_data($data) {
        ...
        $allowedphototiles = get_config('format_eabctiles', 'allowphototiles');
        $tilestyle = get_config('format_eabctiles', 'eabctilestyle') ?? self::TILE_STYLE_STANDARD;
        $data['isstyle-' . $tilestyle] = true;
        $data['eabctilestyle'] = $tilestyle;

        if ($allowedphototiles) {
            $data['allowphototiles'] = 1;
            $data['showprogressphototiles'] = get_config('format_eabctiles', 'showprogresssphototiles');
        ...
        $uselinebreakfilter = get_config('format_eabctiles', 'enablelinebreakfilter');
        ...
        foreach ($secsall as $sectionnum => $section) {
            ...
            if ($countincludedsections >= $maxallowedsections) {
                if (!$data['canedit']) {
                    ...
                    if (!$sectioncountwarningissued) {
                        ...
                        \core\notification::error(get_string('coursetoomanysections', 'format_eabctiles', $a));
            ...
            if ($sectionnum != 0 && $showsection && !$isdelegated) {
                ...
                $ariatitle = get_string('tilearialabel', 'format_eabctiles',
                    trim($title)
                        ? $title
                        : get_string('tilearialabel', 'format_eabctiles', $this->format->get_default_section_name($section))
                );
        ...
        if ($this->courseformatoptions['displayfilterbar']) {
            ...
            if ($usingnumbersfilter) {
                $data['filternumberedbuttons'] = filters::get_filter_numbered_buttons_data($data['eabctiles']);
            }
            if ($usingoutcomesfilter) {
                ...
                $data['filteroutcomebuttons'] = filters::get_filter_outcome_buttons_data($data['eabctiles'], $outcomes, $firstid);
            }
        }
        ...
        if ($this->completionenabled && $data['overall_progress']['num_out_of'] > 0) {
            if (get_config('format_eabctiles', 'showoverallprogress')) {
    ...
    public function section_progress(int $sectionnum): array {
        ...
        if (!empty($sectioncmids)) {
            ...
            $includesubsectiondata = get_config('format_eabctiles', 'progressincludesubsections');
    ...
    private function course_module_data($mod, $section, $previouswaslabel, $isfirst, $output): array {
        ...
        if (!$treataslabel) {
            ...
            if (!\format_eabctiles\local\util::has_monologo_icon('mod', $mod->modname)) {
        ...
        if (\format_eabctiles\local\modal_helper::mod_uses_cm_modal_cache($mod->modname)) {
            $moduleobject['modalType'] = in_array($mod->id, $this->modalscmids)
                ? \format_eabctiles\local\modal_helper::cm_modal_type($this->course->id, $mod->id)
                : null;
        }
        ...
        if ($mod->modname == 'url'&& \format_eabctiles\local\video_cm::is_video_cm($this->course->id, $mod->id)) {
            $externalurl = $DB->get_field('url', 'externalurl', ['id' => $mod->instance]);
            $modifiedvideourl = \format_eabctiles\local\video_cm::check_modify_embedded_url($externalurl);
            if ($modifiedvideourl) {
                // Even though it's really a URL activity, display it as "video" activity with video icon.
                $videostring = get_string('displaytitle_mod_mp4', 'format_eabctiles');
                if ($this->courseformatoptions['courseusesubtiles']) {
                    $moduleobject['extraclasses'] .= ' video';
                    $moduleobject['modnameDisplay'] = $videostring;
                }
                $moduleobject['icon'] = [
                    'url' => $output->image_url("play", 'format_eabctiles'),
                    'label' => $videostring,
                ];
            }
        }
            ...
            if ($mod->completion == COMPLETION_TRACKING_MANUAL) {
                ...
                switch ($completiondata->completionstate) {
                    case COMPLETION_INCOMPLETE:
                        $moduleobject['completionstring'] = get_string('togglecompletionincomplete', 'format_eabctiles');
                        break;
                    case COMPLETION_COMPLETE:
                        $moduleobject['completionstring'] = get_string('togglecompletioncomplete', 'format_eabctiles');
                        break;
                }
            } else { // Automatic.
                switch ($completiondata->completionstate) {
                    case COMPLETION_INCOMPLETE:
                        $moduleobject['completionstring'] = get_string('complete-n-auto', 'format_eabctiles');
                        break;
                    case COMPLETION_COMPLETE:
                        $moduleobject['completionstring'] = get_string('complete-y-auto', 'format_eabctiles');
                        break;
    ...
    public function get_above_tiles_controls(bool $jsnavadminallowed, bool $usingjsnav): array {
        ...
        if ($jsnavadminallowed) {
            $controls[] = [
                'url' => new \moodle_url($courseurl, array_merge($courseurlparams, ['format-eabctiles-action' => 'toggleanimatednav'])),
                'label' => get_string('jsactivate', 'format_eabctiles'),
                'checked' => $usingjsnav,
            ];
        }
        if (get_config('format_eabctiles', 'highcontrastmodeallow')) {
            $controls[] = [
                'url' => new \moodle_url(
                    $courseurl, array_merge($courseurlparams, ['format-eabctiles-action' => 'togglehighcontrast'])
                ),
                'label' => get_string('highcontrastmode', 'format_eabctiles'),
                'checked' => \format_eabctiles\local\util::using_high_contrast(),
            ];
        }
        return $controls;
    }
