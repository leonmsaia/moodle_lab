...
class renderer extends section_renderer {
    ...
    public function render_content() {
        ...
        if ($this->page->user_is_editing()) {
            ...
            $template = 'format_eabctiles/local/content';
            ...
            if (self::display_multiple_section_page((bool)$sectionnumber, false)) {
                $template = 'format_eabctiles/multi_section_page';
                $templateable = new \format_eabctiles\output\course_output($course, false, null, $this);
                $data = $templateable->export_for_template($this);
            } else {
                $template = 'format_eabctiles/single_section_page';
                $templateable = new \format_eabctiles\output\course_output($course, false, $sectionnumber, $this);
                $data = $templateable->export_for_template($this);
            }
        }
        ...
        \format_eabctiles\local\util::init_js($course, $this->page->context->id, $sectionnumber);
     ...
     * @see \format_eabctiles\output\section_renderer::render_section()
     * @see \format_eabctiles\output\courseformat\content\section\controlmenu::export_for_template()
    ...
    public function course_section_updated(\core_courseformat\base $format, \section_info $section): string {
        ...
        $renderer = new \format_eabctiles\output\section_renderer($this->page, $this->target);

        // This ends up calling \format_eabctiles\output\section_renderer::render_section().
    ...
    private function display_multiple_section_page(bool $displaysection, bool $isediting): bool {
        ...
        // We know they have JS if $SESSION->format_eabctiles_jssuccessfullyused is set.
        ...
        if (optional_param('singlesec', 0, PARAM_INT)) {
            // Singlesec param is appended to inplace editable links by format_eabctiles\inplace_editable_render_section_name().
            return false;
        }

        // Otherwise, even if URL requests single, we may show multiple in certain situations.
        if (\format_eabctiles\local\util::using_js_nav() && isset($SESSION->format_eabctiles_jssuccessfullyused)) {
            if (!$isediting && get_config('format_eabctiles', 'usejsnavforsinglesection')) {
                return true;
            }
        }
        return false;
    ...
    public function render_delegatedsection($widget) {
        ...
        $templateable = new \format_eabctiles\output\course_output(
            $this->page->course, true, $sectionnum
        );
        $data = $templateable->export_for_template($this);
        $template = 'format_eabctiles/course_modules_subsection';
        ...
        if ($usecore) {
            ...
            $template = 'format_eabctiles/local/content/delegatedsection';
        }
