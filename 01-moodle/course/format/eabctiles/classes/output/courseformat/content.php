...
namespace format_eabctiles\output\courseformat;
...
class content extends content_base {
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        if ($isediting) {
            ...
            if (get_config('format_eabctiles', 'allowsubtilesview')
                ...
                $messgage = get_string('editoradvicesubtiles', 'format_eabctiles');
                if (has_capability('moodle/site:config', \context_system::instance())) {
                    $messgage .= ' ('
                        . get_string('version', 'format_eabctiles', \format_eabctiles\local\util::get_tiles_plugin_release()) . ')';
                }
            ...
            if ($warneditorcompletion) {
                $bulklink = \html_writer::link(
                  ...
                  get_string('completionwarning_changeinbulk', 'format_eabctiles')
                );
                ...
                $data->editoradvice[] = [
                    'text' => get_string('completionwarning', 'format_eabctiles') . ' '  . $bulklink . ' ' . $helplink,
                    'icon' => 'info-circle', 'class' => 'secondary',
                ];
            }

            $hasbulkedittools = \format_eabctiles\local\util::get_moodle_release() >= 4.2
            ...
            if ($moodlerelease >= 4.0 && \format_eabctiles\local\format_option::needs_migration_incomplete_warning($course->id)) {
    ...
    private function get_inline_folder_warning(int $courseid): ?string {
        ...
        if ($inlinefolder) {
            ...
            return get_string('folderdisplayerror', 'format_eabctiles', $link);
        }
        return null;
    }
}
