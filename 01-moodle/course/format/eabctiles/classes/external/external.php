...
class external extends external_api {
        ...
        switch ($data['imagetype']) {
            case 'tileicon':
                if ($data['sectionid'] === 0) {
                    ...
                    return [
                        'status' => (bool)$result,
                        'iconurl' => $result ? $OUTPUT->image_url('tileicon/' . $data['image'], 'format_eabctiles')->out() : '',
                    ];
                }
            case 'tilephoto':
    ...
    private static function set_tile_photo($data, $context): array {
        ...
        if (!$data['image']) {
            ...
            \format_eabctiles\local\tile_photo::delete_files_from_ids($data['courseid'], $sectionid);

            // If there is an icon attached to this element, clear it (as in this function we are setting a photo).
            \format_eabctiles\local\format_option::unset(
                $data['courseid'], \format_eabctiles\local\format_option::OPTION_SECTION_ICON, $sectionid
            );
    ...
    private static function set_tile_icon($data, $context) {
        ...
        $tilenumber = \format_eabctiles\local\util::get_tile_number_from_icon_name($data['image']);
        if (!$tilenumber) {
            $availableicons = (new \format_eabctiles\local\icon_set)->available_tile_icons($data['courseid']);
            if (!isset($availableicons[$data['image']])) {
                throw new invalid_parameter_exception('Icon is invalid');
            }
        }

        $optiontype = \format_eabctiles\local\format_option::OPTION_SECTION_ICON;
        $sectionid = $data['sectionid'];
        ...
        $defaulticonthiscourse = $DB->get_field(
            ...
            ['courseid' => $data['courseid'], 'format' => 'eabctiles', 'sectionid' => 0, 'name' => 'defaulttileicon']
        );
    ...
    public static function set_default_tileicon($data) {
        ...
        $existingicon = $DB->get_record(
            ...
            ['courseid' => $data['courseid'], 'format' => 'eabctiles', 'sectionid' => $data['sectionid'], 'name' => $optionname]
        );
        if (!isset($existingicon->value)) {
            // No icon is presently stored for this so we need to insert new record.
            $record = new \stdClass();
            $record->courseid = $data['courseid'];
            $record->format = 'eabctiles';
    ...
    public static function log_tile_click(int $coursecontextid, int $sectionnumber, int $sectionid) {
        ...
        $SESSION->format_eabctiles_jssuccessfullyused = 1;
    ...
    public static function get_icon_set($courseid) {
        ...
        $data = [
            'status' => true,
            'warnings' => [],
            'icons' => json_encode((new \format_eabctiles\local\icon_set)->available_tile_icons($params['courseid'])),
            'photos' => '',
        ];
        if (get_config('format_eabctiles', 'allowphototiles')) {
     ...
     * @see \format_eabctiles\local\util::width_template_data() for where this is used.
     * @return array of warnings and status result
     * @since Moodle 3.0
     * @throws moodle_exception
     */
    public static function set_session_width($courseid, $width) {
        ...
        self::validate_context($coursecontext);
        $sessionvar = 'format_eabctiles_width_' . $params['courseid'];

        if (!get_config('format_eabctiles', 'fittilestowidth')) {
    ...
    public static function get_section_information($courseid, $sectionnums) {
        ...
        $templateable = new \format_eabctiles\output\course_output($course, true);
    ...
    public static function get_course_mod_info(int $cmid) {
        ...
        $data = \format_eabctiles\local\util::get_course_mod_info($context->get_course_context()->instanceid, $params['cmid']);
        if ($data) {
            return $data;
        }
