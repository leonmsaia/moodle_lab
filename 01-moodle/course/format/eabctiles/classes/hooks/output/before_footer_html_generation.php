...
namespace format_eabctiles\hooks\output;
...
class before_footer_html_generation {
    ...
    public static function callback(\core\hook\output\before_footer_html_generation $hook): void {
        global $PAGE;
        if (($PAGE->course->format ?? null) !== 'eabctiles') {
        ...
        try {
            ...
            if (($oncourseviewpage && !$editing) || $modviewpageneedsjs) {
                ...
                if (!empty($allowedmodals['resources'] || !empty($allowedmodals['modules']))) {
                    ...
                    if ($launchmodalcmid) {
                        ...
                        $modalallowed =
                            \format_eabctiles\local\util::get_course_mod_info($PAGE->course->id, $launchmodalcmid)->modalallowed
                                ?? false;
                        if (!$modalallowed) {
                            $launchmodalcmid = null;
                        }
                ...
                $PAGE->requires->js_call_amd(
                    'format_eabctiles/course_mod_modal', 'init',
                [$PAGE->course->id, false, $PAGE->pagetype, $launchmodalcmid, \format_eabctiles\local\util::using_js_nav()]
                );
            }
            ...
            if ($PAGE->state === \moodle_page::STATE_IN_BODY) {
                $jsconfig = \format_eabctiles\local\util::get_js_config_data($PAGE->course->id, $allowedmodals ?? []);
                $renderer = $PAGE->get_renderer('format_eabctiles');
                $hook->add_html($renderer->render_from_template('format_eabctiles/js-config', ['eabctiles_js_config' => $jsconfig]));
            }

        } catch (\Exception $e) {
            debugging("Could not prepare format_eabctiles footer data: " . $e->getMessage(), DEBUG_DEVELOPER);
        }
    }
}
