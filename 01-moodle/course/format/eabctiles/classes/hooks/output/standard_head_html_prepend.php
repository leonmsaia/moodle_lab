...
class standard_head_html_prepend {
    ...
    public static function callback(\core\hook\output\before_standard_head_html_generation $hook): void {
        ...
        try {
            if (method_exists('format_eabctiles\local\dynamic_styles', 'get_tiles_dynamic_css')) {
                // The method get_tiles_dynamic_css() will check that we are on a page that really needs it.
                $dynamiccss = \format_eabctiles\local\dynamic_styles::get_tiles_dynamic_css();
                if ($dynamiccss) {
                    $hook->add_html("<style id=\"format-eabctiles-dynamic-css\">$dynamiccss</style>");
                }
            }
        } catch (\Exception $e) {
            debugging("Could not prepare format_eabctiles head data: " . $e->getMessage(), DEBUG_DEVELOPER);
        }
