...
$pageurl = new moodle_url('/course/format/eabctiles/editor/migratecoursedata.php', $pageurlparams);
$PAGE->set_url($pageurl);
$PAGE->set_context($pagecontext);

if ($courseid) {
    $courseurl = new moodle_url('/course/view.php', ['id' => $courseid]);
    $course = $DB->get_record('course', ['id' => $courseid, 'format' => 'eabctiles']);
    if (!$course) {
    ...
    $countlegacyoptions = $DB->get_field_sql(
        "SELECT COUNT(cfo.id) FROM {course_format_options} cfo
                WHERE cfo.courseid = ? AND  cfo.format = 'eabctiles'
    ...
    if ($sesskey) {
        require_sesskey();
        \format_eabctiles\local\format_option::migrate_legacy_format_options($courseid);
        \core\notification::success(
            get_string('migratedcourseid', 'format_eabctiles', $courseid)
        ...
        echo html_writer::tag(
            'h4', get_string('suremigratelegacyoptions', 'format_eabctiles', $countlegacyoptions),
            ['class' => 'mb-3']
        );
...
$PAGE->navbar->add(get_string('migratecoursedata', 'format_eabctiles'));

$legacycourses = $DB->get_records_sql(
"SELECT * FROM
        (SELECT c.id as courseid, c.fullname,
            (SELECT COUNT(cfo.id) FROM {course_format_options} cfo
                WHERE cfo.courseid = c.id AND  cfo.format = 'eabctiles' AND cfo.name IN('tilephoto', 'tileicon')) as legacyoptions,
            (SELECT COUNT(tfo.id) FROM {format_eabctiles_tile_options} tfo
                WHERE tfo.courseid = c.id AND tfo.optiontype IN (?, ?)) as newoptions
        FROM {course} c
         ) counts
    WHERE counts.legacyoptions > 0",
    [\format_eabctiles\local\format_option::OPTION_SECTION_PHOTO, \format_eabctiles\local\format_option::OPTION_SECTION_ICON]
);
$table = new html_table();
$table->head = [
    get_string('course'),
    get_string('legacytiledata', 'format_eabctiles'),
    get_string('newtiledata', 'format_eabctiles'),
    get_string('migratenow', 'format_eabctiles'),
];
$table->data = [];
...
echo html_writer::div(get_string('unmigratedcoursesintro', 'format_eabctiles',  count($legacycourses)), 'mb-2');
