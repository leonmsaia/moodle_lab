...
use format_eabctiles\local\course_section_manager;
...
$pageurl = new moodle_url('/course/format/eabctiles/editor/admintools.php', ['action' => $action]);
$settingsurl = new moodle_url('/admin/settings.php', ['section' => 'formatsettingeabctiles']);

$PAGE->set_url($pageurl);
$PAGE->set_context($systemcontext);
$PAGE->set_heading(get_string('admintools', 'format_eabctiles'));
...
$PAGE->navbar->add(get_string('pluginname', 'format_eabctiles'), $settingsurl);
$PAGE->navbar->add(get_string('admintools', 'format_eabctiles'));
...
function permitted_colours() {
    ...
    $records = $DB->get_records_select(
        'config_plugins',
        "plugin = 'format_eabctiles' AND " . $DB->sql_like('name', '?', false), ["tilecolour%"]
    );
    $permittedcolours = [];
...
function reset_colours($settingsurl, $pageurl) {
    ...
    if (count($permittedcolours) === 0) {
        redirect(
            $settingsurl,
            get_string('novaliddefaultcolour', 'format_eabctiles'), null, \core\output\notification::NOTIFY_ERROR
        );
    }
    ...
    if (!optional_param('sure', 0, PARAM_INT)) {
        ...
        $requiredchangecount = $DB->count_records_sql(
            "SELECT COUNT(courseid) FROM {course_format_options}
                WHERE format = 'eabctiles' AND name = 'basecolour'
                AND value " . $permittedcolourssql,
            $params
        );

        if ($requiredchangecount === 0) {
            redirect($settingsurl, get_string('allcoursescomplypalette', 'format_eabctiles'));
        } else {
            $pageurl->param('sure', '1');
            $pageurl->param('sesskey', sesskey());
            $o = html_writer::div(get_string('sureresetcolours', 'format_eabctiles', $requiredchangecount), 'mb-3 mt-3');
            $o .= html_writer::link($pageurl, get_string('resetcolours', 'format_eabctiles'), ['class' => 'btn btn-danger']);
        ...
        $defaultvalue = get_config('format_eabctiles', 'tilecolour1');
        ...
        $sql = "UPDATE {course_format_options} SET value = :defaultvalue
            WHERE format = 'eabctiles' AND name = 'basecolour' AND sectionid = '0'
            AND value " . $permittedcolourssql;
        ...
        redirect(
            $settingsurl,
            get_string('tilecolourschanged', 'format_eabctiles'), null, \core\output\notification::NOTIFY_SUCCESS
        );
    }
    return '';
}
...
function list_problem_courses() {
    ...
    $o = html_writer::tag(
        ...
        . ' (' . get_string('experimental', 'format_eabctiles') . ')'
    );
    ...
    $countlegacyoptions = $DB->get_field_sql(
        "SELECT COUNT(cfo.id)
                ...
                WHERE cfo.format = 'eabctiles'
                AND cfo.name IN('tilephoto', 'tileicon')"
    );

    $o .= !$countlegacyoptions ? get_string('noproblemsfound', 'format_eabctiles') :
        html_writer::div(
            html_writer::link(
                new moodle_url('/course/format/eabctiles/editor/migratecoursedata.php'),
                get_string('migratecoursedata', 'format_eabctiles'),
