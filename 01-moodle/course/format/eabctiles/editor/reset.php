...
if (optional_param('sesskey', '', PARAM_TEXT)) {
    require_sesskey();
    $result = \format_eabctiles\local\tile_photo::reset_tiles_course($courseid);
    redirect(
        new moodle_url('/course/view.php', ['id' => $courseid]),
        $result ? get_string('eabctilesreset', 'format_eabctiles')
            : get_string('resetincomplete', 'format_eabctiles') ,
        0,
        $result
    ...
    $PAGE->set_url(new moodle_url('/course/format/eabctiles/editor/reset.php', ['id' => $courseid]));
    $PAGE->set_context(context_course::instance($courseid));
    $course = get_course($courseid);
    $PAGE->set_heading($course->fullname);

    $message = html_writer::div(
    html_writer::div(get_string('resetalltilessure', 'format_eabctiles'), 'mb-2')
        . html_writer::link(
            new moodle_url('/course/format/eabctiles/editor/reset.php', ['courseid' => $courseid, 'sesskey' => sesskey()]),
