 ...
 * @return string
 */
function theme_mutualseguridad_get_main_scss_content($theme) {
    global $CFG;
    ...
    // mutualseguridad scss.
    $mutualseguridadvariables = file_get_contents($CFG->dirroot . '/theme/mutualseguridad/scss/mutualseguridad/_variables.scss');
    $mutualseguridad = file_get_contents($CFG->dirroot . '/theme/mutualseguridad/scss/default.scss');
    $security = file_get_contents($CFG->dirroot . '/theme/mutualseguridad/scss/mutualseguridad/_security.scss');

    $lastpreset = '';
    if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_mutualseguridad', 'preset', 0, '/', $filename))) {
        $lastpreset = $presetfile->get_content();
    }

    // Combine them together.
    $allscss = $mutualseguridadvariables . "\n" . $scss . "\n" . $mutualseguridad . "\n" . $lastpreset .    "\n" . $security;
 ...
 * @return string
 */
function theme_mutualseguridad_get_extra_scss($theme) {
    $content = '';
    ...
    if (empty($loginbgimgurl)) {
        $loginbgimgurl = new \moodle_url('/theme/mutualseguridad/pix/loginbg.png');
        $loginbgimgurl->out();
    }
 ...
 * @return string
 */
function theme_mutualseguridad_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 */
// function theme_mutualseguridad_get_precompiled_css() {
//     global $CFG;

//     return file_get_contents($CFG->dirroot . '/theme/mutualseguridad/style/moodle.css');
// }
 ...
 * @return mixed
 */
function theme_mutualseguridad_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    $theme = theme_config::load('mutualseguridad');
    ...
    if ($filearea === 'hvp') {
        return theme_mutualseguridad_serve_hvp_css($args[1], $theme);
    }
 ...
 */
function theme_mutualseguridad_serve_hvp_css($filename, $theme) {
    global $CFG, $PAGE;
    ...
    $settings = new \theme_mutualseguridad\util\settings();
    $content = $settings->hvpcss;

    $md5content = md5($content);
    $md5stored = get_config('theme_mutualseguridad', 'hvpccssmd5');
